...
class EnhancedExtractor:
    ...
    def normalize_text(self, text: str) -> str:
        ...
        # v6.6 (PDF-eval): the SAME "¼"-for-"=" font glyph also lands AFTER the CI
        # keyword ("95% CI ¼ 1.55 to 2.05", "confidence interval ¼ 0.68 to 0.90"),
        # which the ratio-term repairs above do not cover -> the point was recovered
        # but both CI bounds were dropped (point_only). Restore "=" ONLY between a CI
        # keyword and a digit, so a real "¼" is never rewritten. Observed across two
        # pancreatic-cancer prognostic-factor review PDFs from one journal font.
