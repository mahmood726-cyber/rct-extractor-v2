"""
Malnutrition extractor validation on the corpus: extraction yield, 2x2 / continuous
yield, and internal-consistency rate over abstracts (and PDFs with --pdfs).

Usage:
  python scripts/malnutrition/validate_malnutrition.py --limit 3000
  python scripts/malnutrition/validate_malnutrition.py --pdfs    # use downloaded full-text PDFs
"""
import argparse
import io
import json
...
from rct_extractor._engine.specialties.malnutrition_arm_data import extract_arm_level
from rct_extractor._engine.specialties.registry import detect_specialty

MALN = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "malnutrition"
MATCHED = MALN / "malnutrition_matched.jsonl"
PDF_DIR = MALN / "rct_trial_pdfs"
OUT = MALN / "validation.json"
...
def main():
    ...
    eff_consistent = 0
    types = Counter(); maln_subspec = Counter()
    ...
    res = {
        ...
        "malnutrition_subspecialty": dict(maln_subspec.most_common()),
        "arm_level": {
            "proportions": n_props,
