"""
Build the malnutrition RCT corpus index from PubMed (reuses the malaria corpus helpers).

Output: data/field_portability/malnutrition/malnutrition_matched.jsonl
  {study_id, pmid, pmcid, doi, nct_id, other_registry_ids, title, journal, year,
   abstract, has_pdf}

Usage:
  python scripts/malnutrition/build_malnutrition_corpus.py --retmax 3000 --email you@org
"""
import argparse
import io
import json
...
OUT_DIR = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "malnutrition"
OUT_FILE = OUT_DIR / "malnutrition_matched.jsonl"

MALNUTRITION_TERM = (
    '("severe acute malnutrition"[Title/Abstract] OR "acute malnutrition"[Title/Abstract] OR '
    'malnutrition[Title/Abstract] OR undernutrition[Title/Abstract] OR '
    '"ready-to-use therapeutic food"[Title/Abstract] OR kwashiorkor[Title/Abstract] OR '
    'marasmus[Title/Abstract] OR wasting[Title/Abstract] OR stunting[Title/Abstract]) '
...
def main():
    ap = argparse.ArgumentParser(description="Build malnutrition RCT corpus index")
    ap.add_argument("--term", default=MALNUTRITION_TERM)
    ...
    print(f"esearch malnutrition RCTs (retmax={args.retmax})...")
