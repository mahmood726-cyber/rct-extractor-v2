"""
Build an AACT gold file of malnutrition trial effect estimates (reuses the malaria
AACT dialect + param_type normalization). Many nutrition trials register on ISRCTN
/ PACTR / CTRI rather than ClinicalTrials.gov, so this gold may be sparse.

Usage:
  python scripts/malnutrition/build_aact_malnutrition_gold.py --aact "F:/AACT-storage/AACT/2026-04-12"
"""
import argparse
import io
import json
...
OUT = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "malnutrition" / "aact_malnutrition_gold.json"
...
def main():
    ...
    SELECT DISTINCT nct_id FROM (
        SELECT nct_id FROM {rc(aact + '/conditions.txt')}
          WHERE downcase_name LIKE '%malnutrition%' OR downcase_name LIKE '%undernutrition%'
             OR downcase_name LIKE '%kwashiorkor%' OR downcase_name LIKE '%marasmus%'
             OR downcase_name LIKE '%wasting%' OR downcase_name LIKE '%stunting%'
        UNION
        SELECT nct_id FROM {rc(aact + '/browse_conditions.txt')}
          WHERE downcase_mesh_term LIKE '%malnutrition%' OR downcase_mesh_term LIKE '%nutrition disorder%'
             OR downcase_mesh_term LIKE '%growth disorder%' OR downcase_mesh_term LIKE '%wasting%'
    )
    """
    ncts = [r[0] for r in con.execute(nct_q).fetchall() if r[0]]
    print(f"malnutrition NCTs in AACT: {len(ncts)}")
    if not ncts:
        OUT.write_text(json.dumps({"source": "AACT", "n_studies": 0, "n_effects": 0,
                                   "n_typed_effects": 0, "studies": []}, indent=2),
                       encoding="utf-8")
        sys.exit("no malnutrition NCTs in AACT")
