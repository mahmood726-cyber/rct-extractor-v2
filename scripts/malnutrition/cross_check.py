"""
Malnutrition unified cross-check: per trial reconcile PDF extractions vs PubMed
abstract vs AACT (when NCT). Abstract->PDF recall is the at-scale signal;
AACT->PDF is the independent numeric gold (sparse for nutrition trials).

Outputs: data/field_portability/malnutrition/cross_check.json
Usage: python scripts/malnutrition/cross_check.py --limit 0
"""
import argparse
import io
import json
...
AACT_GOLD = MALN / "aact_malnutrition_gold.json"
PDF_DIR = MALN / "rct_trial_pdfs"
OUT = MALN / "cross_check.json"
