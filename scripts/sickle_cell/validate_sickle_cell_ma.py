"""
Validate the sickle cell extractor against PUBLISHED sickle cell meta-analyses
(silver standard).

Reuses the malaria MA-validation machinery: for each published sickle cell RCT
...
Usage:
  python scripts/sickle_cell/validate_sickle_cell_ma.py --retmax 250 --email you@org
"""
import argparse
import json
import sys
...
OUT = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "sickle_cell" / "ma_validation.json"

SICKLE_CELL_MA_TERM = (
    '("sickle cell" OR "sickle-cell") AND '
    '(meta-analysis[Publication Type] OR systematic review[Publication Type]) '
    'AND (hydroxyurea OR transfusion OR voxelotor OR crizanlizumab OR randomized '
    'OR randomised OR efficacy OR trial)'
)
...
def main():
    ...
    pmids = esearch_pmids(SICKLE_CELL_MA_TERM, args.retmax, args.email)
    print(f"sickle cell MA PMIDs: {len(pmids)}")
