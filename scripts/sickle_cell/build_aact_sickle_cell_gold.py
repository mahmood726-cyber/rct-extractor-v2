"""
Build an AACT gold file of sickle cell trial effect estimates (reuses the malaria
AACT dialect + param_type normalization). Sickle cell disease registers well on
ClinicalTrials.gov, so this gold may be richer than the malaria / typhoid ones.

Usage:
  python scripts/sickle_cell/build_aact_sickle_cell_gold.py --aact "F:/AACT-storage/AACT/2026-04-12"
"""
import argparse
import io
import json
...
OUT = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "sickle_cell" / "aact_sickle_cell_gold.json"
...
def main():
    ...
    SELECT DISTINCT nct_id FROM (
        SELECT nct_id FROM {rc(aact + '/conditions.txt')}
          WHERE downcase_name LIKE '%sickle cell%' OR downcase_name LIKE '%sickle-cell%'
             OR downcase_name LIKE '%hemoglobin ss%' OR downcase_name LIKE '%haemoglobin ss%'
             OR downcase_name LIKE '%hemoglobin sc%'
        UNION
        SELECT nct_id FROM {rc(aact + '/browse_conditions.txt')}
          WHERE downcase_mesh_term LIKE '%sickle cell%' OR downcase_mesh_term LIKE '%hemoglobin sc%'
             OR downcase_mesh_term LIKE '%anemia, sickle cell%'
    )
    """
    ncts = [r[0] for r in con.execute(nct_q).fetchall() if r[0]]
    print(f"sickle cell NCTs in AACT: {len(ncts)}")
    if not ncts:
        OUT.write_text(json.dumps({"source": "AACT", "n_studies": 0, "n_effects": 0,
                                   "n_typed_effects": 0, "studies": []}, indent=2),
                       encoding="utf-8")
        sys.exit("no sickle cell NCTs in AACT")
