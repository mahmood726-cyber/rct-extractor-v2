"""
Build the sickle cell disease RCT corpus index from PubMed (reuses the malaria
corpus helpers).

Output: data/field_portability/sickle_cell/sickle_cell_matched.jsonl
  {study_id, pmid, pmcid, doi, nct_id, other_registry_ids, title, journal, year,
   abstract, has_pdf}

Usage:
  python scripts/sickle_cell/build_sickle_cell_corpus.py --retmax 6000 --email you@org
"""
import argparse
import io
import json
...
OUT_DIR = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "sickle_cell"
OUT_FILE = OUT_DIR / "sickle_cell_matched.jsonl"

SICKLE_CELL_TERM = (
    '("sickle cell"[Title/Abstract] OR "sickle-cell"[Title/Abstract] OR '
    '"sickle cell disease"[Title/Abstract] OR "sickle cell anaemia"[Title/Abstract] OR '
    '"sickle cell anemia"[Title/Abstract] OR "haemoglobin SS"[Title/Abstract] OR '
    '"hemoglobin SS"[Title/Abstract] OR "vaso-occlusive"[Title/Abstract]) '
...
def main():
    ap = argparse.ArgumentParser(description="Build sickle cell RCT corpus index")
    ap.add_argument("--term", default=SICKLE_CELL_TERM)
    ...
    print(f"esearch sickle cell RCTs (retmax={args.retmax})...")
