"""
Sickle cell extractor validation on the corpus: extraction yield, 2x2 / continuous
yield, and internal-consistency rate over abstracts (and PDFs with --pdfs).

Usage:
  python scripts/sickle_cell/validate_sickle_cell.py --limit 6000
  python scripts/sickle_cell/validate_sickle_cell.py --pdfs        # use downloaded full-text PDFs
"""
import argparse
import io
import json
...
from rct_extractor._engine.specialties.sickle_cell_arm_data import extract_arm_level
from rct_extractor._engine.specialties.registry import detect_specialty

SCD = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "sickle_cell"
MATCHED = SCD / "sickle_cell_matched.jsonl"
PDF_DIR = SCD / "rct_trial_pdfs"
OUT = SCD / "validation.json"
...
def main():
    ...
    eff_consistent = 0
    types = Counter(); scd_subspec = Counter()
    ...
    res = {
        ...
        "sickle_cell_subspecialty": dict(scd_subspec.most_common()),
        "arm_level": {
            "proportions": n_props,
