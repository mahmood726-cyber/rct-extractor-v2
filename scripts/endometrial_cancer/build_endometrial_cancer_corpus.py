"""
Build the endometrial cancer RCT corpus index from PubMed (reuses malaria helpers).
Output: data/field_portability/endometrial_cancer/endometrial_cancer_matched.jsonl
Usage: python scripts/endometrial_cancer/build_endometrial_cancer_corpus.py --retmax 3000
"""
...
OUT_DIR = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "endometrial_cancer"
OUT_FILE = OUT_DIR / "endometrial_cancer_matched.jsonl"

ENDOMETRIAL_CANCER_TERM = (
    '("endometrial cancer"[Title/Abstract] OR "endometrial carcinoma"[Title/Abstract] '
    'OR "uterine cancer"[Title/Abstract] OR "uterine carcinoma"[Title/Abstract] '
    'OR "endometrial adenocarcinoma"[Title/Abstract]) '
...
def main():
    ap = argparse.ArgumentParser(description="Build endometrial cancer RCT corpus index")
    ap.add_argument("--term", default=ENDOMETRIAL_CANCER_TERM)
    ...
    print(f"esearch endometrial cancer RCTs (retmax={args.retmax})...")
    ...
    for bi, batch in enumerate(chunks(pmids, args.batch)):
        ...
        for pmid in batch:
            ...
            ids = idmap.get(pmid, {})
            records[pmid] = {
                "study_id": f"endometrial_cancer_{pmid}", "pmid": pmid,
