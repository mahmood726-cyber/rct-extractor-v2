"""
Build the COVID-19 RCT corpus index from PubMed (reuses malaria corpus helpers).
Canonical NCBI query is COVID19_TERM. NCBI eutils was DNS-blocked on the build
host; real-PDF gold acquired via the EuropePMC harness
(scripts/pdf_eval/acquire_epmc_gold.py). Builder kept for parity.
Output: data/field_portability/covid19/covid19_matched.jsonl
"""
...
OUT_DIR = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "covid19"
OUT_FILE = OUT_DIR / "covid19_matched.jsonl"
COVID19_TERM = (
...
def main():
    ap = argparse.ArgumentParser(description="Build COVID-19 RCT corpus index")
    ap.add_argument("--term", default=COVID19_TERM)
