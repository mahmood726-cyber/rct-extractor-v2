"""
Build the hypertension RCT corpus index from PubMed (reuses the malaria corpus helpers).

Output: data/field_portability/hypertension/hypertension_matched.jsonl
  {study_id, pmid, pmcid, doi, nct_id, other_registry_ids, title, journal, year,
   abstract, has_pdf}

Hypertension is a very large field (tens of thousands of RCTs), so --retmax is a
SAMPLE cap, not the true population count (unlike typhoid). Increase it for a
...
OUT_DIR = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "hypertension"
OUT_FILE = OUT_DIR / "hypertension_matched.jsonl"

HYPERTENSION_TERM = (
    '(hypertension[Title/Abstract] OR "high blood pressure"[Title/Abstract] OR '
    'antihypertensive[Title/Abstract] OR "blood pressure lowering"[Title/Abstract] OR '
    '"blood pressure control"[Title/Abstract]) '
...
def main():
    ap = argparse.ArgumentParser(description="Build hypertension RCT corpus index")
    ap.add_argument("--term", default=HYPERTENSION_TERM)
    ...
    print(f"esearch hypertension RCTs (retmax={args.retmax})...")
