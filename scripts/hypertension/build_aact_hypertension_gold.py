"""
Build an AACT gold file of hypertension trial effect estimates (reuses the malaria
AACT dialect + param_type normalization). Hypertension is heavily represented on
ClinicalTrials.gov with posted results, so this gold is expected to be RICHER than
typhoid/malaria — a genuine external numeric validation set.

Usage:
  python scripts/hypertension/build_aact_hypertension_gold.py --aact "F:/AACT-storage/AACT/2026-04-12"
"""
import argparse
import io
import json
...
OUT = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "hypertension" / "aact_hypertension_gold.json"
...
def main():
    ...
    SELECT DISTINCT nct_id FROM (
        SELECT nct_id FROM {rc(aact + '/conditions.txt')}
          WHERE downcase_name LIKE '%hypertension%' OR downcase_name LIKE '%high blood pressure%'
             OR downcase_name LIKE '%hypertensive%'
        UNION
        SELECT nct_id FROM {rc(aact + '/browse_conditions.txt')}
          WHERE downcase_mesh_term LIKE '%hypertension%' OR downcase_mesh_term LIKE '%hypertensive%'
    )
    """
    ncts = [r[0] for r in con.execute(nct_q).fetchall() if r[0]]
    print(f"hypertension NCTs in AACT: {len(ncts)}")
    if not ncts:
        OUT.write_text(json.dumps({"source": "AACT", "n_studies": 0, "n_effects": 0,
                                   "n_typed_effects": 0, "studies": []}, indent=2),
                       encoding="utf-8")
        sys.exit("no hypertension NCTs in AACT")
