"""
Hypertension unified cross-check: per trial reconcile PDF extractions vs PubMed
abstract vs AACT (when NCT). Abstract->PDF recall is the at-scale signal;
AACT->PDF is the independent numeric gold. Hypertension registers heavily on
ClinicalTrials.gov, so AACT gold is expected to be richer than typhoid/malaria.

Outputs: data/field_portability/hypertension/cross_check.json
Usage: python scripts/hypertension/cross_check.py --limit 0
"""
import argparse
import io
import json
...
HTN = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "hypertension"
MATCHED = HTN / "hypertension_matched.jsonl"
AACT_GOLD = HTN / "aact_hypertension_gold.json"
PDF_DIR = HTN / "rct_trial_pdfs"
OUT = HTN / "cross_check.json"
