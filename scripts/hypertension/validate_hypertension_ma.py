"""
Validate the hypertension extractor against PUBLISHED antihypertensive
meta-analyses (silver standard).

Reuses the malaria MA-validation machinery: for each published hypertension RCT
...
OUT = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "hypertension" / "ma_validation.json"

HYPERTENSION_MA_TERM = (
    '(hypertension OR "blood pressure" OR antihypertensive) AND '
    '(meta-analysis[Publication Type] OR systematic review[Publication Type]) '
    'AND (antihypertensive OR "blood pressure" OR randomized OR randomised '
    'OR efficacy OR trial OR cardiovascular)'
)
