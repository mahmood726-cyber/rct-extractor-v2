"""
Validate the cholera extractor against PUBLISHED cholera meta-analyses (silver
standard).

Reuses the malaria MA-validation machinery: for each published cholera RCT
...
Usage:
  python scripts/cholera/validate_cholera_ma.py --retmax 150 --email you@org
"""
import argparse
import json
import sys
...
CHOLERA_MA_TERM = (
    '(cholera OR "Vibrio cholerae") AND '
    '(meta-analysis[Publication Type] OR systematic review[Publication Type]) '
    'AND (antibiotic OR vaccine OR rehydration OR randomized OR randomised '
...
def main():
    ...
    pmids = esearch_pmids(CHOLERA_MA_TERM, args.retmax, args.email)
    print(f"cholera MA PMIDs: {len(pmids)}")
