"""
Build the cholera RCT corpus index from PubMed (reuses the malaria corpus helpers).

Output: data/field_portability/cholera/cholera_matched.jsonl
  {study_id, pmid, pmcid, doi, nct_id, other_registry_ids, title, journal, year,
   abstract, has_pdf}

Usage:
  python scripts/cholera/build_cholera_corpus.py --retmax 3000 --email you@org
"""
import argparse
import io
import json
...
OUT_DIR = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "cholera"
OUT_FILE = OUT_DIR / "cholera_matched.jsonl"

CHOLERA_TERM = (
    '(cholera[Title/Abstract] OR "Vibrio cholerae"[Title/Abstract] OR '
    '"oral cholera vaccine"[Title/Abstract] OR vibriocidal[Title/Abstract]) '
...
def main():
    ap = argparse.ArgumentParser(description="Build cholera RCT corpus index")
    ap.add_argument("--term", default=CHOLERA_TERM)
    ...
    print(f"esearch cholera RCTs (retmax={args.retmax})...")
