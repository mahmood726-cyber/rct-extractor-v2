"""
Cholera extractor validation on the corpus: extraction yield, 2x2 / continuous
yield, and internal-consistency rate over abstracts (and PDFs with --pdfs).

Usage:
  python scripts/cholera/validate_cholera.py --limit 3000
  python scripts/cholera/validate_cholera.py --pdfs        # use downloaded full-text PDFs
"""
import argparse
import io
import json
...
from rct_extractor._engine.specialties.cholera_arm_data import extract_arm_level
from rct_extractor._engine.specialties.registry import detect_specialty

CHOLERA = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "cholera"
MATCHED = CHOLERA / "cholera_matched.jsonl"
PDF_DIR = CHOLERA / "rct_trial_pdfs"
OUT = CHOLERA / "validation.json"
...
def main():
    ...
    eff_consistent = 0
    types = Counter(); cholera_subspec = Counter()
    ...
    res = {
        ...
        "cholera_subspecialty": dict(cholera_subspec.most_common()),
        "arm_level": {
            "proportions": n_props,
