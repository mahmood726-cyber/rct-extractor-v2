"""
Meningitis unified cross-check: per trial reconcile PDF extractions vs PubMed
abstract vs AACT (when NCT). Abstract->PDF recall is the at-scale signal;
AACT->PDF is the independent numeric gold (vaccine-trial-heavy for meningitis).

Outputs: data/field_portability/meningitis/cross_check.json
Usage: python scripts/meningitis/cross_check.py --limit 0
"""
import argparse
import io
import json
...
AACT_GOLD = MENINGITIS / "aact_meningitis_gold.json"
PDF_DIR = MENINGITIS / "rct_trial_pdfs"
OUT = MENINGITIS / "cross_check.json"
