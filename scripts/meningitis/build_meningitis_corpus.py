"""
Build the meningitis RCT corpus index from PubMed (reuses the malaria corpus helpers).

Output: data/field_portability/meningitis/meningitis_matched.jsonl
  {study_id, pmid, pmcid, doi, nct_id, other_registry_ids, title, journal, year,
   abstract, has_pdf}

Usage:
  python scripts/meningitis/build_meningitis_corpus.py --retmax 4000 --email you@org
"""
import argparse
import io
import json
...
OUT_DIR = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "meningitis"
OUT_FILE = OUT_DIR / "meningitis_matched.jsonl"

MENINGITIS_TERM = (
    '(meningitis[Title/Abstract] OR meningococcal[Title/Abstract] OR '
    '"Neisseria meningitidis"[Title/Abstract] OR "pneumococcal meningitis"[Title/Abstract] '
    'OR "meningococcal vaccine"[Title/Abstract] OR "meningococcal conjugate vaccine"[Title/Abstract]) '
...
def main():
    ap = argparse.ArgumentParser(description="Build meningitis RCT corpus index")
    ap.add_argument("--term", default=MENINGITIS_TERM)
    ...
    print(f"esearch meningitis RCTs (retmax={args.retmax})...")
