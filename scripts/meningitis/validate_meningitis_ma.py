"""
Validate the meningitis extractor against PUBLISHED meningitis meta-analyses
(silver standard).

Reuses the malaria MA-validation machinery: for each published meningitis RCT
...
Usage:
  python scripts/meningitis/validate_meningitis_ma.py --retmax 200 --email you@org
"""
import argparse
import json
import sys
...
OUT = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "meningitis" / "ma_validation.json"

MENINGITIS_MA_TERM = (
    '(meningitis OR meningococcal OR "bacterial meningitis") AND '
    '(meta-analysis[Publication Type] OR systematic review[Publication Type]) '
    'AND (antibiotic OR vaccine OR dexamethasone OR randomized OR randomised OR efficacy OR trial)'
)
...
def main():
    ...
    pmids = esearch_pmids(MENINGITIS_MA_TERM, args.retmax, args.email)
    print(f"meningitis MA PMIDs: {len(pmids)}")
