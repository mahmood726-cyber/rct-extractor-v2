"""
Validate the maternal & neonatal extractor against PUBLISHED maternal/neonatal
meta-analyses (silver standard).

Reuses the malaria MA-validation machinery: for each published maternal/neonatal
RCT meta-analysis, treat every reported `value (95% CI lo-hi)` as a reviewer datum
and measure how many our extractor recovers (point AND CI agree). Separates
comparative effect estimates (HR/OR/RR/MD) from prevalences/proportions.

Usage:
  python scripts/maternal_neonatal/validate_maternal_neonatal_ma.py --retmax 200 --email you@org
"""
import argparse
import json
import sys
...
OUT = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "maternal_neonatal" / "ma_validation.json"

MNH_MA_TERM = (
    '("postpartum haemorrhage" OR "postpartum hemorrhage" OR "pre-eclampsia" OR '
    'preeclampsia OR eclampsia OR "maternal mortality" OR "neonatal mortality" OR '
    'stillbirth OR "preterm birth" OR "low birth weight" OR "neonatal sepsis") AND '
    '(meta-analysis[Publication Type] OR systematic review[Publication Type]) '
    'AND (randomized OR randomised OR trial OR efficacy OR intervention)'
)
...
def main():
    ...
    pmids = esearch_pmids(MNH_MA_TERM, args.retmax, args.email)
    print(f"maternal/neonatal MA PMIDs: {len(pmids)}")
