"""
Build an AACT gold file of maternal/neonatal trial effect estimates (reuses the
malaria AACT dialect + param_type normalization). Many maternal/neonatal trials
in LMIC settings register on ISRCTN / CTRI / PACTR rather than ClinicalTrials.gov,
so this gold may be sparser than the cardiology / HIV golds.

Usage:
  python scripts/maternal_neonatal/build_aact_maternal_neonatal_gold.py --aact "F:/AACT-storage/AACT/2026-04-12"
"""
import argparse
import io
import json
...
OUT = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "maternal_neonatal" / "aact_maternal_neonatal_gold.json"
...
def main():
    ...
    SELECT DISTINCT nct_id FROM (
        SELECT nct_id FROM {rc(aact + '/conditions.txt')}
          WHERE downcase_name LIKE '%postpartum hemorrhage%' OR downcase_name LIKE '%postpartum haemorrhage%'
             OR downcase_name LIKE '%pre-eclampsia%' OR downcase_name LIKE '%preeclampsia%'
             OR downcase_name LIKE '%eclampsia%' OR downcase_name LIKE '%maternal mortality%'
             OR downcase_name LIKE '%neonatal mortality%' OR downcase_name LIKE '%stillbirth%'
             OR downcase_name LIKE '%preterm birth%' OR downcase_name LIKE '%premature birth%'
             OR downcase_name LIKE '%low birth weight%' OR downcase_name LIKE '%neonatal sepsis%'
             OR downcase_name LIKE '%birth asphyxia%' OR downcase_name LIKE '%perinatal mortality%'
        UNION
        SELECT nct_id FROM {rc(aact + '/browse_conditions.txt')}
          WHERE downcase_mesh_term LIKE '%postpartum hemorrhage%' OR downcase_mesh_term LIKE '%pre-eclampsia%'
             OR downcase_mesh_term LIKE '%eclampsia%' OR downcase_mesh_term LIKE '%premature birth%'
             OR downcase_mesh_term LIKE '%infant, low birth weight%' OR downcase_mesh_term LIKE '%stillbirth%'
             OR downcase_mesh_term LIKE '%neonatal sepsis%' OR downcase_mesh_term LIKE '%asphyxia neonatorum%'
    )
    """
    ncts = [r[0] for r in con.execute(nct_q).fetchall() if r[0]]
    print(f"maternal/neonatal NCTs in AACT: {len(ncts)}")
    if not ncts:
        OUT.write_text(json.dumps({"source": "AACT", "n_studies": 0, "n_effects": 0,
                                   "n_typed_effects": 0, "studies": []}, indent=2),
                       encoding="utf-8")
        sys.exit("no maternal/neonatal NCTs in AACT")
