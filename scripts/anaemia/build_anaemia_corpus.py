"""
Build the anaemia RCT corpus index from PubMed (reuses malaria helpers).

Output: data/field_portability/anaemia/anaemia_matched.jsonl

Usage:
  python scripts/anaemia/build_anaemia_corpus.py --retmax 4000 --email you@org
"""
import argparse
import io
import json
...
OUT_DIR = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "anaemia"
OUT_FILE = OUT_DIR / "anaemia_matched.jsonl"

ANAEMIA_TERM = (
    '("iron deficiency anemia"[Title/Abstract] OR "iron deficiency anaemia"[Title/Abstract] '
    'OR anemia[Title/Abstract] OR anaemia[Title/Abstract] '
    'OR "intravenous iron"[Title/Abstract] OR "ferric carboxymaltose"[Title/Abstract] '
    'OR "erythropoiesis-stimulating"[Title/Abstract] OR darbepoetin[Title/Abstract] '
    'OR roxadustat[Title/Abstract] OR "hemoglobin response"[Title/Abstract] '
    'OR "red blood cell transfusion"[Title/Abstract]) '
...
def main():
    ap = argparse.ArgumentParser(description="Build anaemia RCT corpus index")
    ap.add_argument("--term", default=ANAEMIA_TERM)
    ...
    print(f"esearch anaemia RCTs (retmax={args.retmax})...")
