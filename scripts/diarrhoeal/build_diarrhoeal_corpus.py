"""
Build the diarrhoeal-disease RCT corpus index from PubMed (reuses the malaria
corpus helpers).

Output: data/field_portability/diarrhoeal/diarrhoeal_matched.jsonl
  {study_id, pmid, pmcid, doi, nct_id, other_registry_ids, title, journal, year,
   abstract, has_pdf}

Usage:
  python scripts/diarrhoeal/build_diarrhoeal_corpus.py --retmax 5000 --email you@org
"""
import argparse
import io
import json
...
OUT_DIR = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "diarrhoeal"
OUT_FILE = OUT_DIR / "diarrhoeal_matched.jsonl"

# Diarrhoeal disease: acute/persistent childhood diarrhoea, rotavirus, ORS/zinc,
# dysentery. Deliberately scoped to the diarrhoeal-disease sibling -- cholera and
# typhoid have their own profiles, so they are not the focus here.
DIARRHOEAL_TERM = (
    '(diarrhoea[Title/Abstract] OR diarrhea[Title/Abstract] OR '
    'gastroenteritis[Title/Abstract] OR rotavirus[Title/Abstract] OR '
    '"oral rehydration"[Title/Abstract] OR dysentery[Title/Abstract] OR '
    'shigellosis[Title/Abstract]) '
...
def main():
    ap = argparse.ArgumentParser(description="Build diarrhoeal RCT corpus index")
    ap.add_argument("--term", default=DIARRHOEAL_TERM)
    ...
    print(f"esearch diarrhoeal RCTs (retmax={args.retmax})...")
