"""
Diarrhoeal extractor validation on the corpus: extraction yield, 2x2 / continuous
yield, and internal-consistency rate over abstracts (and PDFs with --pdfs).

Usage:
  python scripts/diarrhoeal/validate_diarrhoeal.py --limit 5000
  python scripts/diarrhoeal/validate_diarrhoeal.py --pdfs    # use downloaded full-text PDFs
"""
import argparse
import io
import json
...
from rct_extractor._engine.specialties.diarrhoeal_arm_data import extract_arm_level
from rct_extractor._engine.specialties.registry import detect_specialty

DIARRHOEAL = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "diarrhoeal"
MATCHED = DIARRHOEAL / "diarrhoeal_matched.jsonl"
PDF_DIR = DIARRHOEAL / "rct_trial_pdfs"
OUT = DIARRHOEAL / "validation.json"
...
def main():
    ...
    eff_consistent = 0
    types = Counter(); diarrhoeal_subspec = Counter()
    ...
    res = {
        ...
        "diarrhoeal_subspecialty": dict(diarrhoeal_subspec.most_common()),
        "arm_level": {
            "proportions": n_props,
