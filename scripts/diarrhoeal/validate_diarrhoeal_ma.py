"""
Validate the diarrhoeal extractor against PUBLISHED diarrhoeal-disease
meta-analyses (silver standard).

Reuses the malaria MA-validation machinery: for each published diarrhoeal RCT
meta-analysis (ORS/zinc, rotavirus vaccine, antibiotics for dysentery), treat
every reported `value (95% CI lo-hi)` as a reviewer datum and measure how many
our extractor recovers (point AND CI agree). Separates comparative effect
estimates (HR/OR/RR/IRR/MD) from prevalences/proportions.

Usage:
  python scripts/diarrhoeal/validate_diarrhoeal_ma.py --retmax 200 --email you@org
"""
import argparse
import json
import sys
...
OUT = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "diarrhoeal" / "ma_validation.json"

DIARRHOEAL_MA_TERM = (
    '(diarrhoea OR diarrhea OR gastroenteritis OR rotavirus OR "oral rehydration" '
    'OR dysentery OR zinc) AND '
    '(meta-analysis[Publication Type] OR systematic review[Publication Type]) '
    'AND (randomized OR randomised OR efficacy OR trial OR vaccine OR zinc)'
)
...
def main():
    ...
    pmids = esearch_pmids(DIARRHOEAL_MA_TERM, args.retmax, args.email)
    print(f"diarrhoeal MA PMIDs: {len(pmids)}")
