"""
Build an AACT gold file of diarrhoeal-disease trial effect estimates (reuses the
malaria AACT dialect + param_type normalization). Many diarrhoeal trials (ORS /
zinc / rotavirus vaccine in LMICs) register on ISRCTN / CTRI / PACTR rather than
ClinicalTrials.gov, so this gold may be sparser than cardiology's.

Usage:
  python scripts/diarrhoeal/build_aact_diarrhoeal_gold.py --aact "F:/AACT-storage/AACT/2026-04-12"
"""
import argparse
import io
import json
...
OUT = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "diarrhoeal" / "aact_diarrhoeal_gold.json"
...
def main():
    ...
    SELECT DISTINCT nct_id FROM (
        SELECT nct_id FROM {rc(aact + '/conditions.txt')}
          WHERE downcase_name LIKE '%diarrhea%' OR downcase_name LIKE '%diarrhoea%'
             OR downcase_name LIKE '%gastroenteritis%' OR downcase_name LIKE '%rotavirus%'
             OR downcase_name LIKE '%dysentery%' OR downcase_name LIKE '%shigell%'
        UNION
        SELECT nct_id FROM {rc(aact + '/browse_conditions.txt')}
          WHERE downcase_mesh_term LIKE '%diarrhea%' OR downcase_mesh_term LIKE '%gastroenteritis%'
             OR downcase_mesh_term LIKE '%rotavirus%' OR downcase_mesh_term LIKE '%dysentery%'
    )
    """
    ncts = [r[0] for r in con.execute(nct_q).fetchall() if r[0]]
    print(f"diarrhoeal NCTs in AACT: {len(ncts)}")
    if not ncts:
        OUT.write_text(json.dumps({"source": "AACT", "n_studies": 0, "n_effects": 0,
                                   "n_typed_effects": 0, "studies": []}, indent=2),
                       encoding="utf-8")
        sys.exit("no diarrhoeal NCTs in AACT")
