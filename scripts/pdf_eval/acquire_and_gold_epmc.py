...
def acquire_and_build(specialty, query, max_search, max_download, target, out_path,
    ...
    # that match the query only in body/references (e.g. a metabolic-syndrome or
    # CMV-infection trial surfacing for an MDS query) with NO disease term in the
    # abstract. Such papers are not in-scope for the disease and would distort
    # accuracy. require_term (a regex) keeps only candidates whose abstract
    # actually concerns the disease. Corpus hygiene, NOT gold tampering: it never
    # edits a harvested value, only which on-disease abstracts are eligible.
    # Default None preserves prior behaviour for every specialty already built.
