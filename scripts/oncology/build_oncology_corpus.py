"""
Build the oncology (breast / lung / colorectal cancer) RCT corpus index from
PubMed (reuses the malaria corpus helpers).

Output: data/field_portability/oncology/oncology_matched.jsonl
  {study_id, pmid, pmcid, doi, title, journal, year, abstract, has_pdf}

Usage:
  python scripts/oncology/build_oncology_corpus.py --retmax 3000 --email you@org
"""
import argparse
import io
import json
...
OUT_DIR = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "oncology"
OUT_FILE = OUT_DIR / "oncology_matched.jsonl"

ONCOLOGY_TERM = (
    '("breast cancer"[Title/Abstract] OR "lung cancer"[Title/Abstract] OR '
    '"non-small cell lung cancer"[Title/Abstract] OR "colorectal cancer"[Title/Abstract]) '
...
def main():
    ap = argparse.ArgumentParser(description="Build oncology RCT corpus index")
    ap.add_argument("--term", default=ONCOLOGY_TERM)
