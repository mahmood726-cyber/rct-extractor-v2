"""
Build the osteoarthritis RCT corpus index from PubMed (reuses malaria corpus
helpers). Canonical NCBI query is OSTEOARTHRITIS_TERM. NCBI eutils was DNS-blocked
...
Output: data/field_portability/osteoarthritis/osteoarthritis_matched.jsonl
"""
...
OUT_DIR = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "osteoarthritis"
OUT_FILE = OUT_DIR / "osteoarthritis_matched.jsonl"
OSTEOARTHRITIS_TERM = (
    '(osteoarthritis[Title/Abstract] OR "knee osteoarthritis"[Title/Abstract] '
    'OR "hip osteoarthritis"[Title/Abstract]) '
...
def main():
    ap = argparse.ArgumentParser(description="Build osteoarthritis RCT corpus index")
    ap.add_argument("--term", default=OSTEOARTHRITIS_TERM)
