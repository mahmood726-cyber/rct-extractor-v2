"""
Pneumonia unified cross-check: per trial reconcile PDF extractions vs PubMed
abstract vs AACT (when NCT). Abstract->PDF recall is the at-scale signal;
AACT->PDF is the independent numeric gold (richer for vaccine trials).

Outputs: data/field_portability/pneumonia/cross_check.json
Usage: python scripts/pneumonia/cross_check.py --limit 0
"""
import argparse
import io
import json
...
AACT_GOLD = PNEUMONIA / "aact_pneumonia_gold.json"
PDF_DIR = PNEUMONIA / "rct_trial_pdfs"
OUT = PNEUMONIA / "cross_check.json"
