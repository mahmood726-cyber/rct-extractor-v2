"""
Build the childhood-pneumonia RCT corpus index from PubMed (reuses the malaria
corpus helpers).

Output: data/field_portability/pneumonia/pneumonia_matched.jsonl
...
OUT_DIR = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "pneumonia"
OUT_FILE = OUT_DIR / "pneumonia_matched.jsonl"

# Childhood pneumonia / ARI RCTs: antibiotic treatment, pneumococcal/Hib vaccines,
# and severe-pneumonia / mortality outcomes, restricted to children where possible.
PNEUMONIA_TERM = (
    '((pneumonia[Title/Abstract] OR "lower respiratory tract infection"[Title/Abstract] '
    'OR "acute respiratory infection"[Title/Abstract] OR pneumococcal[Title/Abstract] '
    'OR "Streptococcus pneumoniae"[Title/Abstract]) '
    'AND (child*[Title/Abstract] OR infant*[Title/Abstract] OR p[a]ediatric[Title/Abstract] '
    'OR "under-five"[Title/Abstract] OR "under five"[Title/Abstract])) '
...
def main():
    ap = argparse.ArgumentParser(description="Build childhood-pneumonia RCT corpus index")
    ap.add_argument("--term", default=PNEUMONIA_TERM)
    ...
    print(f"esearch pneumonia RCTs (retmax={args.retmax})...")
