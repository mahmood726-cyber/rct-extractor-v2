"""
Build an AACT gold file of childhood-pneumonia trial effect estimates (reuses the
malaria AACT dialect + param_type normalization). Many pneumonia / vaccine trials
register on ClinicalTrials.gov, but African ARI treatment trials often register on
ISRCTN / PACTR, so this gold may be sparser than the vaccine subset.

Usage:
  python scripts/pneumonia/build_aact_pneumonia_gold.py --aact "F:/AACT-storage/AACT/2026-04-12"
"""
import argparse
import io
import json
...
OUT = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "pneumonia" / "aact_pneumonia_gold.json"
...
def main():
    ...
    SELECT DISTINCT nct_id FROM (
        SELECT nct_id FROM {rc(aact + '/conditions.txt')}
          WHERE downcase_name LIKE '%pneumonia%' OR downcase_name LIKE '%pneumococc%'
             OR downcase_name LIKE '%lower respiratory%'
             OR downcase_name LIKE '%respiratory tract infection%'
        UNION
        SELECT nct_id FROM {rc(aact + '/browse_conditions.txt')}
          WHERE downcase_mesh_term LIKE '%pneumonia%'
             OR downcase_mesh_term LIKE '%pneumococcal infection%'
    )
    """
    ncts = [r[0] for r in con.execute(nct_q).fetchall() if r[0]]
    print(f"pneumonia NCTs in AACT: {len(ncts)}")
    if not ncts:
        OUT.write_text(json.dumps({"source": "AACT", "n_studies": 0, "n_effects": 0,
                                   "n_typed_effects": 0, "studies": []}, indent=2),
                       encoding="utf-8")
        sys.exit("no pneumonia NCTs in AACT")
