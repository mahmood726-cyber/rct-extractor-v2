"""
Validate the pneumonia extractor against PUBLISHED childhood-pneumonia
meta-analyses (silver standard).

Reuses the malaria MA-validation machinery: for each published pneumonia RCT
...
Usage:
  python scripts/pneumonia/validate_pneumonia_ma.py --retmax 200 --email you@org
"""
import argparse
import json
import sys
...
OUT = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "pneumonia" / "ma_validation.json"

PNEUMONIA_MA_TERM = (
    '(pneumonia OR "lower respiratory tract infection" OR pneumococcal) AND '
    '(child* OR infant* OR paediatric OR pediatric OR "under-five") AND '
...
def main():
    ...
    pmids = esearch_pmids(PNEUMONIA_MA_TERM, args.retmax, args.email)
    print(f"pneumonia MA PMIDs: {len(pmids)}")
