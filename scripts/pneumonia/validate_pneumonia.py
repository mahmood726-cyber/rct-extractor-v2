"""
Pneumonia extractor validation on the corpus: extraction yield, 2x2 / continuous
yield, and internal-consistency rate over abstracts (and PDFs with --pdfs).

Usage:
  python scripts/pneumonia/validate_pneumonia.py --limit 5000
  python scripts/pneumonia/validate_pneumonia.py --pdfs        # use downloaded full-text PDFs
"""
import argparse
import io
import json
...
from rct_extractor._engine.specialties.pneumonia_arm_data import extract_arm_level
from rct_extractor._engine.specialties.registry import detect_specialty

PNEUMONIA = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "pneumonia"
MATCHED = PNEUMONIA / "pneumonia_matched.jsonl"
PDF_DIR = PNEUMONIA / "rct_trial_pdfs"
OUT = PNEUMONIA / "validation.json"
...
def main():
    ...
    eff_consistent = 0
    types = Counter(); pneumonia_subspec = Counter()
    ...
    res = {
        ...
        "pneumonia_subspecialty": dict(pneumonia_subspec.most_common()),
        "arm_level": {
            "proportions": n_props,
