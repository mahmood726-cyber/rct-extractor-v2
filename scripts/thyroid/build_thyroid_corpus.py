"""
Build the thyroid-disorders RCT corpus index from PubMed
(reuses the malaria corpus helpers).

Output: data/field_portability/thyroid/thyroid_matched.jsonl
...
retained for the canonical eutils pipeline and exposes THYROID_TERM.

Usage:
  python scripts/thyroid/build_thyroid_corpus.py --retmax 4000
"""
import argparse
import io
import json
...
OUT_DIR = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "thyroid"
OUT_FILE = OUT_DIR / "thyroid_matched.jsonl"

THYROID_TERM = (
    '(hypothyroidism[Title/Abstract] OR hyperthyroidism[Title/Abstract] OR '
    'thyroid[Title/Abstract] OR levothyroxine[Title/Abstract] OR '
    '"Graves disease"[Title/Abstract] OR methimazole[Title/Abstract] OR '
    'carbimazole[Title/Abstract] OR thyroiditis[Title/Abstract] OR '
    'thyrotoxicosis[Title/Abstract]) '
...
def main():
    ap = argparse.ArgumentParser(description="Build thyroid RCT corpus index")
    ap.add_argument("--term", default=THYROID_TERM)
    ...
    print(f"esearch thyroid RCTs (retmax={args.retmax})...")
