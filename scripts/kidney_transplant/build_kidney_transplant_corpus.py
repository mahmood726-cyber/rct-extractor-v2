"""
Build the kidney-transplant RCT corpus index from PubMed (reuses malaria helpers).

Output: data/field_portability/kidney_transplant/kidney_transplant_matched.jsonl
...
retained for the canonical eutils pipeline and exposes KIDNEY_TRANSPLANT_TERM.

Usage:
  python scripts/kidney_transplant/build_kidney_transplant_corpus.py --retmax 4000
"""
import argparse
import io
import json
...
OUT_DIR = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "kidney_transplant"
OUT_FILE = OUT_DIR / "kidney_transplant_matched.jsonl"

KIDNEY_TRANSPLANT_TERM = (
    '("kidney transplant"[Title/Abstract] OR "kidney transplantation"[Title/Abstract] OR '
    '"renal transplant"[Title/Abstract] OR "renal transplantation"[Title/Abstract] OR '
    '"renal allograft"[Title/Abstract] OR "kidney allograft"[Title/Abstract]) '
...
def main():
    ap = argparse.ArgumentParser(description="Build kidney-transplant RCT corpus index")
    ap.add_argument("--term", default=KIDNEY_TRANSPLANT_TERM)
    ...
    print(f"esearch kidney-transplant RCTs (retmax={args.retmax})...")
