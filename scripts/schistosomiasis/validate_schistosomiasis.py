"""
Schistosomiasis extractor validation on the corpus: extraction yield, 2x2 /
continuous yield, and internal-consistency rate over abstracts (and PDFs with
--pdfs).

Usage:
  python scripts/schistosomiasis/validate_schistosomiasis.py --limit 4000
  python scripts/schistosomiasis/validate_schistosomiasis.py --pdfs   # use downloaded full-text PDFs
"""
import argparse
import io
import json
...
from rct_extractor._engine.specialties.schistosomiasis_arm_data import extract_arm_level
from rct_extractor._engine.specialties.registry import detect_specialty

SCHISTO = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "schistosomiasis"
MATCHED = SCHISTO / "schistosomiasis_matched.jsonl"
PDF_DIR = SCHISTO / "rct_trial_pdfs"
OUT = SCHISTO / "validation.json"
...
def main():
    ...
    eff_consistent = 0
    types = Counter(); schisto_subspec = Counter()
    ...
    for r in rows:
        ...
        if spec == "schistosomiasis":
            schisto_subspec[sub] += 1
    ...
    res = {
        ...
        "schistosomiasis_subspecialty": dict(schisto_subspec.most_common()),
        "arm_level": {
            "proportions": n_props,
