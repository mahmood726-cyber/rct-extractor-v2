"""
Validate the schistosomiasis extractor against PUBLISHED schistosomiasis
meta-analyses (silver standard).

Reuses the malaria MA-validation machinery: for each published schistosomiasis RCT
...
Usage:
  python scripts/schistosomiasis/validate_schistosomiasis_ma.py --retmax 200 --email you@org
"""
import argparse
import json
import sys
...
OUT = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "schistosomiasis" / "ma_validation.json"

SCHISTO_MA_TERM = (
    '(schistosomiasis OR schistosoma OR bilharzia) AND '
    '(meta-analysis[Publication Type] OR systematic review[Publication Type]) '
    'AND (praziquantel OR antischistosomal OR vaccine OR randomized OR randomised '
    'OR efficacy OR trial)'
)
...
def main():
    ...
    pmids = esearch_pmids(SCHISTO_MA_TERM, args.retmax, args.email)
    print(f"schistosomiasis MA PMIDs: {len(pmids)}")
