"""
Build the schistosomiasis RCT corpus index from PubMed (reuses the malaria corpus helpers).

Output: data/field_portability/schistosomiasis/schistosomiasis_matched.jsonl
  {study_id, pmid, pmcid, doi, nct_id, other_registry_ids, title, journal, year,
   abstract, has_pdf}

Usage:
  python scripts/schistosomiasis/build_schistosomiasis_corpus.py --retmax 4000 --email you@org
"""
import argparse
import io
import json
...
OUT_DIR = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "schistosomiasis"
OUT_FILE = OUT_DIR / "schistosomiasis_matched.jsonl"

SCHISTO_TERM = (
    '(schistosomiasis[Title/Abstract] OR schistosoma[Title/Abstract] OR '
    'bilharzia[Title/Abstract] OR praziquantel[Title/Abstract] OR '
    '"Schistosoma mansoni"[Title/Abstract] OR "Schistosoma haematobium"[Title/Abstract] OR '
    '"Schistosoma japonicum"[Title/Abstract]) '
...
def main():
    ap = argparse.ArgumentParser(description="Build schistosomiasis RCT corpus index")
    ap.add_argument("--term", default=SCHISTO_TERM)
    ...
    print(f"esearch schistosomiasis RCTs (retmax={args.retmax})...")
