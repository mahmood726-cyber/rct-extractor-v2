"""
Build the low-back-pain RCT corpus index from PubMed (reuses malaria helpers).

Output: data/field_portability/low_back_pain/low_back_pain_matched.jsonl

Usage:
  python scripts/low_back_pain/build_low_back_pain_corpus.py --retmax 4000 --email you@org
"""
import argparse
import io
import json
...
OUT_DIR = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "low_back_pain"
OUT_FILE = OUT_DIR / "low_back_pain_matched.jsonl"

LOW_BACK_PAIN_TERM = (
    '("low back pain"[Title/Abstract] OR "chronic low back pain"[Title/Abstract] '
    'OR "acute low back pain"[Title/Abstract] OR lumbago[Title/Abstract] '
    'OR sciatica[Title/Abstract] OR "Oswestry disability"[Title/Abstract] '
    'OR "Roland-Morris"[Title/Abstract] OR "lumbar disc herniation"[Title/Abstract] '
    'OR "non-specific back pain"[Title/Abstract]) '
...
def main():
    ap = argparse.ArgumentParser(description="Build low-back-pain RCT corpus index")
    ap.add_argument("--term", default=LOW_BACK_PAIN_TERM)
    ...
    print(f"esearch low-back-pain RCTs (retmax={args.retmax})...")
