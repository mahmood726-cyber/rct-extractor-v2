"""
Trachoma extractor validation on the corpus: extraction yield, 2x2 / continuous
yield, and internal-consistency rate over abstracts (and PDFs with --pdfs).

Usage:
  python scripts/trachoma/validate_trachoma.py --limit 4000
  python scripts/trachoma/validate_trachoma.py --pdfs   # use downloaded full-text PDFs
"""
import argparse
import io
import json
...
from src.specialties.trachoma_arm_data import extract_arm_level
from src.specialties.registry import detect_specialty

TRACHOMA = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "trachoma"
MATCHED = TRACHOMA / "trachoma_matched.jsonl"
PDF_DIR = TRACHOMA / "rct_trial_pdfs"
OUT = TRACHOMA / "validation.json"
...
def main():
    ...
    eff_consistent = 0
    types = Counter(); trachoma_subspec = Counter()
    ...
    res = {
        ...
        "trachoma_subspecialty": dict(trachoma_subspec.most_common()),
        "arm_level": {
            "proportions": n_props,
