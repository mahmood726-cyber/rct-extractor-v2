"""
Validate the trachoma extractor against PUBLISHED trachoma meta-analyses
(silver standard).

Reuses the malaria MA-validation machinery: for each published trachoma RCT
...
Usage:
  python scripts/trachoma/validate_trachoma_ma.py --retmax 200 --email you@org
"""
import argparse
import json
import sys
...
OUT = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "trachoma" / "ma_validation.json"

TRACHOMA_MA_TERM = (
    '(trachoma OR trachomatous OR trichiasis) AND '
    '(meta-analysis[Publication Type] OR systematic review[Publication Type]) '
    'AND (azithromycin OR antibiotic OR surgery OR SAFE OR "mass drug administration" '
...
def main():
    ...
    pmids = esearch_pmids(TRACHOMA_MA_TERM, args.retmax, args.email)
    print(f"trachoma MA PMIDs: {len(pmids)}")
