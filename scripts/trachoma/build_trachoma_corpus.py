"""
Build the trachoma RCT corpus index from PubMed (reuses the malaria corpus helpers).

Output: data/field_portability/trachoma/trachoma_matched.jsonl
  {study_id, pmid, pmcid, doi, nct_id, other_registry_ids, title, journal, year,
   abstract, has_pdf}

Usage:
  python scripts/trachoma/build_trachoma_corpus.py --retmax 4000 --email you@org
"""
import argparse
import io
import json
...
OUT_DIR = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "trachoma"
OUT_FILE = OUT_DIR / "trachoma_matched.jsonl"

TRACHOMA_TERM = (
    '(trachoma[Title/Abstract] OR trachomatous[Title/Abstract] OR '
    'trichiasis[Title/Abstract] OR "ocular chlamydia"[Title/Abstract] OR '
    '"trachomatous trichiasis"[Title/Abstract] OR "active trachoma"[Title/Abstract]) '
...
def main():
    ap = argparse.ArgumentParser(description="Build trachoma RCT corpus index")
    ap.add_argument("--term", default=TRACHOMA_TERM)
    ...
    print(f"esearch trachoma RCTs (retmax={args.retmax})...")
