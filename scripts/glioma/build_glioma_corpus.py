"""
Build the glioma/glioblastoma RCT corpus index from PubMed (reuses malaria helpers).
Output: data/field_portability/glioma/glioma_matched.jsonl
Usage: python scripts/glioma/build_glioma_corpus.py --retmax 3000
"""
...
OUT_DIR = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "glioma"
OUT_FILE = OUT_DIR / "glioma_matched.jsonl"

GLIOMA_TERM = (
    '("glioblastoma"[Title/Abstract] OR "glioma"[Title/Abstract] '
    'OR "astrocytoma"[Title/Abstract] OR "oligodendroglioma"[Title/Abstract] '
    'OR "high-grade glioma"[Title/Abstract]) '
...
def main():
    ap = argparse.ArgumentParser(description="Build glioma RCT corpus index")
    ap.add_argument("--term", default=GLIOMA_TERM)
    ...
    print(f"esearch glioma RCTs (retmax={args.retmax})...")
    ...
    for bi, batch in enumerate(chunks(pmids, args.batch)):
        ...
        for pmid in batch:
            ...
            ids = idmap.get(pmid, {})
            records[pmid] = {
                "study_id": f"glioma_{pmid}", "pmid": pmid,
