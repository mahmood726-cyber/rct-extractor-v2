"""
Build the melanoma RCT corpus index from PubMed (reuses malaria helpers).
Output: data/field_portability/melanoma/melanoma_matched.jsonl
Usage: python scripts/melanoma/build_melanoma_corpus.py --retmax 3000
"""
...
OUT_DIR = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "melanoma"
OUT_FILE = OUT_DIR / "melanoma_matched.jsonl"

MELANOMA_TERM = (
    '("melanoma"[Title/Abstract] OR "cutaneous melanoma"[Title/Abstract] '
    'OR "metastatic melanoma"[Title/Abstract] OR "malignant melanoma"[Title/Abstract] '
    'OR "uveal melanoma"[Title/Abstract]) '
...
def main():
    ap = argparse.ArgumentParser(description="Build melanoma RCT corpus index")
    ap.add_argument("--term", default=MELANOMA_TERM)
    ...
    print(f"esearch melanoma RCTs (retmax={args.retmax})...")
    ...
    for bi, batch in enumerate(chunks(pmids, args.batch)):
        ...
        for pmid in batch:
            ...
            ids = idmap.get(pmid, {})
            records[pmid] = {
                "study_id": f"melanoma_{pmid}", "pmid": pmid,
