"""
Build the osteoporosis RCT corpus index from PubMed (reuses the malaria helpers).

Output: data/field_portability/osteoporosis/osteoporosis_matched.jsonl
...
retained for the canonical eutils pipeline and exposes OSTEOPOROSIS_TERM.

Usage:
  python scripts/osteoporosis/build_osteoporosis_corpus.py --retmax 4000
"""
import argparse
import io
import json
...
OUT_DIR = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "osteoporosis"
OUT_FILE = OUT_DIR / "osteoporosis_matched.jsonl"

OSTEOPOROSIS_TERM = (
    '(osteoporosis[Title/Abstract] OR "bone mineral density"[Title/Abstract] OR '
    '"vertebral fracture"[Title/Abstract] OR "fragility fracture"[Title/Abstract] OR '
    'bisphosphonate[Title/Abstract] OR alendronate[Title/Abstract] OR '
    'denosumab[Title/Abstract] OR teriparatide[Title/Abstract] OR '
    'romosozumab[Title/Abstract]) '
...
def main():
    ap = argparse.ArgumentParser(description="Build osteoporosis RCT corpus index")
    ap.add_argument("--term", default=OSTEOPOROSIS_TERM)
    ...
    print(f"esearch osteoporosis RCTs (retmax={args.retmax})...")
