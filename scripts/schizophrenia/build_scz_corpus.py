"""
Build the schizophrenia RCT corpus index from PubMed (reuses malaria corpus
helpers). Canonical NCBI query is SCHIZOPHRENIA_TERM. NCBI eutils was DNS-blocked
on the build host; real-PDF gold acquired via the EuropePMC harness
(scripts/pdf_eval/acquire_epmc_gold.py). Builder kept for parity.

Output: data/field_portability/schizophrenia/schizophrenia_matched.jsonl
"""
...
OUT_DIR = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "schizophrenia"
OUT_FILE = OUT_DIR / "schizophrenia_matched.jsonl"
SCHIZOPHRENIA_TERM = (
    '(schizophrenia[Title/Abstract] OR schizoaffective[Title/Abstract] '
    'OR "psychotic disorder"[Title/Abstract]) '
    'AND (randomized controlled trial[Publication Type] '
    'OR randomized[Title/Abstract] OR randomised[Title/Abstract])'
)
def main():
    ap = argparse.ArgumentParser(description="Build schizophrenia RCT corpus index")
    ap.add_argument("--term", default=SCHIZOPHRENIA_TERM)
    ...
    if not pmids: sys.exit("no PMIDs")
    records = {}
    for batch in chunks(pmids, args.batch):
        meta = efetch_records(batch, args.email); idmap = idconv_batch(batch, args.email)
        for pmid in batch:
            m = meta.get(pmid)
            if not m: continue
            ids = idmap.get(pmid, {})
