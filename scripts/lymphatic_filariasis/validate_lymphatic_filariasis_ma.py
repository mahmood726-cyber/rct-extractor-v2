"""
Validate the lymphatic filariasis (LF) extractor against PUBLISHED LF / filariasis
meta-analyses (silver standard).

Reuses the malaria MA-validation machinery: for each published LF RCT
...
Usage:
  python scripts/lymphatic_filariasis/validate_lymphatic_filariasis_ma.py --retmax 250 --email you@org
"""
import argparse
import json
import sys
...
OUT = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "lymphatic_filariasis" / "ma_validation.json"

LF_MA_TERM = (
    '("lymphatic filariasis" OR filariasis OR wuchereria OR brugia OR '
    'microfilaraemia OR microfilaremia OR diethylcarbamazine OR ivermectin OR '
    'albendazole) AND (meta-analysis[Publication Type] OR '
    'systematic review[Publication Type]) AND (microfilaria OR clearance OR '
    'efficacy OR antigenaemia OR randomized OR randomised OR trial)'
)


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--retmax", type=int, default=250)
    ...
    pmids = esearch_pmids(LF_MA_TERM, args.retmax, args.email)
    print(f"LF MA PMIDs: {len(pmids)}")
