"""
Lymphatic filariasis (LF) extractor validation on the corpus: extraction yield,
2x2 / continuous yield, and internal-consistency rate over abstracts (and PDFs
with --pdfs).

Usage:
  python scripts/lymphatic_filariasis/validate_lymphatic_filariasis.py --limit 4000
  python scripts/lymphatic_filariasis/validate_lymphatic_filariasis.py --pdfs   # full-text PDFs
"""
import argparse
import io
import json
...
from src.specialties.lymphatic_filariasis_arm_data import extract_arm_level
from src.specialties.registry import detect_specialty

LF = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "lymphatic_filariasis"
MATCHED = LF / "lymphatic_filariasis_matched.jsonl"
PDF_DIR = LF / "rct_trial_pdfs"
OUT = LF / "validation.json"
...
def main():
    ...
    eff_consistent = 0
    types = Counter(); lf_subspec = Counter()
    ...
    res = {
        ...
        "lymphatic_filariasis_subspecialty": dict(lf_subspec.most_common()),
        "arm_level": {
            "proportions": n_props,
