"""
Build the respiratory (COPD / asthma / ILD) RCT corpus index from PubMed
(reuses the malaria corpus helpers).

Output: data/field_portability/respiratory/respiratory_matched.jsonl
  {study_id, pmid, pmcid, doi, nct_id, other_registry_ids, title, journal, year,
   abstract, has_pdf}

Usage:
  python scripts/respiratory/build_respiratory_corpus.py --retmax 3000 --email you@org
"""
import argparse
import io
import json
...
OUT_DIR = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "respiratory"
OUT_FILE = OUT_DIR / "respiratory_matched.jsonl"

RESPIRATORY_TERM = (
    '("chronic obstructive pulmonary disease"[Title/Abstract] OR COPD[Title/Abstract] OR '
    'asthma[Title/Abstract] OR "idiopathic pulmonary fibrosis"[Title/Abstract] OR '
...
def main():
    ap = argparse.ArgumentParser(description="Build respiratory RCT corpus index")
    ap.add_argument("--term", default=RESPIRATORY_TERM)
