"""
Onchocerciasis extractor validation on the corpus: extraction yield, 2x2 /
continuous yield, and internal-consistency rate over abstracts (and PDFs with
--pdfs).

Usage:
  python scripts/onchocerciasis/validate_onchocerciasis.py --limit 4000
  python scripts/onchocerciasis/validate_onchocerciasis.py --pdfs   # use downloaded full-text PDFs
"""
import argparse
import io
import json
...
from src.specialties.onchocerciasis_arm_data import extract_arm_level
from src.specialties.registry import detect_specialty

ONCHO = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "onchocerciasis"
MATCHED = ONCHO / "onchocerciasis_matched.jsonl"
PDF_DIR = ONCHO / "rct_trial_pdfs"
OUT = ONCHO / "validation.json"
...
def main():
    ...
    eff_consistent = 0
    types = Counter(); oncho_subspec = Counter()
    ...
    res = {
        ...
        "onchocerciasis_subspecialty": dict(oncho_subspec.most_common()),
        "arm_level": {
            "proportions": n_props,
