"""
Build an AACT gold file of onchocerciasis trial effect estimates (reuses the
malaria AACT dialect + param_type normalization). Many onchocerciasis trials
register on ISRCTN / PACTR rather than ClinicalTrials.gov, so this gold may be
sparser than HIV's.

Usage:
  python scripts/onchocerciasis/build_aact_onchocerciasis_gold.py --aact "F:/AACT-storage/AACT/2026-04-12"
"""
import argparse
import io
import json
...
OUT = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "onchocerciasis" / "aact_onchocerciasis_gold.json"
...
def main():
    ...
    SELECT DISTINCT nct_id FROM (
        SELECT nct_id FROM {rc(aact + '/conditions.txt')}
          WHERE downcase_name LIKE '%onchocerc%' OR downcase_name LIKE '%river blindness%'
        UNION
        SELECT nct_id FROM {rc(aact + '/browse_conditions.txt')}
          WHERE downcase_mesh_term LIKE '%onchocerc%'
    )
    """
    ncts = [r[0] for r in con.execute(nct_q).fetchall() if r[0]]
    print(f"onchocerciasis NCTs in AACT: {len(ncts)}")
    if not ncts:
        OUT.write_text(json.dumps({"source": "AACT", "n_studies": 0, "n_effects": 0,
                                   "n_typed_effects": 0, "studies": []}, indent=2),
                       encoding="utf-8")
        sys.exit("no onchocerciasis NCTs in AACT")
