"""
Validate the onchocerciasis extractor against PUBLISHED onchocerciasis
meta-analyses (silver standard).

Reuses the malaria MA-validation machinery: for each published onchocerciasis RCT
...
Usage:
  python scripts/onchocerciasis/validate_onchocerciasis_ma.py --retmax 200 --email you@org
"""
import argparse
import json
import sys
...
OUT = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "onchocerciasis" / "ma_validation.json"

ONCHO_MA_TERM = (
    '(onchocerciasis OR onchocerca OR "river blindness") AND '
    '(meta-analysis[Publication Type] OR systematic review[Publication Type]) '
    'AND (ivermectin OR moxidectin OR doxycycline OR microfilar* OR '
    'efficacy OR randomized OR randomised OR trial)'
)
...
def main():
    ...
    pmids = esearch_pmids(ONCHO_MA_TERM, args.retmax, args.email)
    print(f"onchocerciasis MA PMIDs: {len(pmids)}")
