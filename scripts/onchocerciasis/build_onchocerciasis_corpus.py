"""
Build the onchocerciasis RCT corpus index from PubMed (reuses the malaria corpus helpers).

Output: data/field_portability/onchocerciasis/onchocerciasis_matched.jsonl
  {study_id, pmid, pmcid, doi, nct_id, other_registry_ids, title, journal, year,
   abstract, has_pdf}

Usage:
  python scripts/onchocerciasis/build_onchocerciasis_corpus.py --retmax 4000 --email you@org
"""
import argparse
import io
import json
...
OUT_DIR = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "onchocerciasis"
OUT_FILE = OUT_DIR / "onchocerciasis_matched.jsonl"

ONCHO_TERM = (
    '(onchocerciasis[Title/Abstract] OR onchocerca[Title/Abstract] OR '
    '"river blindness"[Title/Abstract] OR moxidectin[Title/Abstract] OR '
    '"Onchocerca volvulus"[Title/Abstract] OR '
    '(ivermectin[Title/Abstract] AND (onchocerc*[Title/Abstract] OR '
    'microfilar*[Title/Abstract] OR "skin snip"[Title/Abstract]))) '
...
def main():
    ap = argparse.ArgumentParser(description="Build onchocerciasis RCT corpus index")
    ap.add_argument("--term", default=ONCHO_TERM)
    ...
    print(f"esearch onchocerciasis RCTs (retmax={args.retmax})...")
