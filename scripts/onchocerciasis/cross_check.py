"""
Onchocerciasis unified cross-check: per trial reconcile PDF extractions vs PubMed
abstract vs AACT (when NCT). Abstract->PDF recall is the at-scale signal;
AACT->PDF is the independent numeric gold (sparse for onchocerciasis).

Outputs: data/field_portability/onchocerciasis/cross_check.json
Usage: python scripts/onchocerciasis/cross_check.py --limit 0
"""
import argparse
import io
import json
...
AACT_GOLD = ONCHO / "aact_onchocerciasis_gold.json"
PDF_DIR = ONCHO / "rct_trial_pdfs"
OUT = ONCHO / "cross_check.json"
