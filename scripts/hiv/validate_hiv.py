"""
HIV extractor validation on the corpus: extraction yield, 2x2 / continuous yield,
and internal-consistency rate over abstracts (and PDFs with --pdfs).

Usage:
  python scripts/hiv/validate_hiv.py --limit 3000
  python scripts/hiv/validate_hiv.py --pdfs        # use downloaded full-text PDFs
"""
import argparse
import io
import json
...
from rct_extractor._engine.specialties.hiv_arm_data import extract_arm_level
from rct_extractor._engine.specialties.registry import detect_specialty

HIV = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "hiv"
MATCHED = HIV / "hiv_matched.jsonl"
PDF_DIR = HIV / "rct_trial_pdfs"
OUT = HIV / "validation.json"
...
def main():
    ...
    eff_consistent = 0
    types = Counter(); hiv_subspec = Counter()
    ...
    res = {
        ...
        "hiv_subspecialty": dict(hiv_subspec.most_common()),
        "arm_level": {
            "proportions": n_props,
