"""
Build an AACT gold file of HIV trial effect estimates (reuses the malaria AACT
dialect + param_type normalization). HIV RCTs are heavily registered with posted
results, so this gold should be far richer than malaria's.

Usage:
  python scripts/hiv/build_aact_hiv_gold.py --aact "F:/AACT-storage/AACT/2026-04-12"
"""
import argparse
import io
import json
...
OUT = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "hiv" / "aact_hiv_gold.json"
...
def main():
    ...
    SELECT DISTINCT nct_id FROM (
        SELECT nct_id FROM {rc(aact + '/conditions.txt')}
          WHERE downcase_name LIKE '%hiv%' OR downcase_name LIKE '%human immunodeficiency%'
             OR downcase_name LIKE '%acquired immunodeficiency%'
        UNION
        SELECT nct_id FROM {rc(aact + '/browse_conditions.txt')}
          WHERE downcase_mesh_term LIKE '%hiv%' OR downcase_mesh_term LIKE '%acquired immunodeficiency%'
    )
    """
    ncts = [r[0] for r in con.execute(nct_q).fetchall() if r[0]]
    print(f"HIV NCTs in AACT: {len(ncts)}")
