"""
Validate the HIV extractor against PUBLISHED HIV meta-analyses (silver standard).

Reuses the malaria MA-validation machinery: for each published HIV RCT
...
Usage:
  python scripts/hiv/validate_hiv_ma.py --retmax 150 --email you@org
"""
import argparse
import json
import sys
...
OUT = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "hiv" / "ma_validation.json"

HIV_MA_TERM = (
    'HIV AND (meta-analysis[Publication Type] OR systematic review[Publication Type]) '
    'AND (antiretroviral OR randomized OR randomised OR efficacy OR trial)'
)
...
def main():
    ...
    pmids = esearch_pmids(HIV_MA_TERM, args.retmax, args.email)
    print(f"HIV MA PMIDs: {len(pmids)}")
