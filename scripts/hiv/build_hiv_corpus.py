"""
Build the HIV RCT corpus index from PubMed (reuses the malaria corpus helpers).

Output: data/field_portability/hiv/hiv_matched.jsonl
  {study_id, pmid, pmcid, doi, nct_id, other_registry_ids, title, journal, year,
   abstract, has_pdf}

Usage:
  python scripts/hiv/build_hiv_corpus.py --retmax 3000 --email you@org
"""
import argparse
import io
import json
...
OUT_DIR = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "hiv"
OUT_FILE = OUT_DIR / "hiv_matched.jsonl"

HIV_TERM = (
    '(HIV[Title/Abstract] OR antiretroviral[Title/Abstract] OR '
    '"pre-exposure prophylaxis"[Title/Abstract]) '
...
def main():
    ap = argparse.ArgumentParser(description="Build HIV RCT corpus index")
    ap.add_argument("--term", default=HIV_TERM)
    ...
    print(f"esearch HIV RCTs (retmax={args.retmax})...")
