"""
Helminths (STH / deworming) extractor validation on the corpus: extraction yield,
...
Usage:
  ...
  python scripts/helminths/validate_helminths.py --pdfs        # use downloaded full-text PDFs
"""
import argparse
import io
import json
...
from rct_extractor._engine.specialties.helminths_arm_data import extract_arm_level
from rct_extractor._engine.specialties.registry import detect_specialty

HELM = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "helminths"
MATCHED = HELM / "helminths_matched.jsonl"
PDF_DIR = HELM / "rct_trial_pdfs"
OUT = HELM / "validation.json"
...
def main():
    ...
    eff_consistent = 0
    types = Counter(); helm_subspec = Counter()
    ...
    res = {
        ...
        "helminths_subspecialty": dict(helm_subspec.most_common()),
        "arm_level": {
            "proportions": n_props,
