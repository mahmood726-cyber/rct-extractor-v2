"""
Validate the soil-transmitted helminths (STH) / deworming extractor against
PUBLISHED deworming / STH meta-analyses (silver standard).

Reuses the malaria MA-validation machinery: for each published deworming RCT
...
Usage:
  python scripts/helminths/validate_helminths_ma.py --retmax 200 --email you@org
"""
import argparse
import json
import sys
...
OUT = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "helminths" / "ma_validation.json"

HELMINTHS_MA_TERM = (
    '("soil-transmitted helminth" OR deworming OR anthelmintic OR albendazole OR '
    'mebendazole OR ascaris OR trichuris OR hookworm OR strongyloides) AND '
    '(meta-analysis[Publication Type] OR systematic review[Publication Type]) '
    'AND (cure OR "egg reduction" OR efficacy OR randomized OR randomised OR trial)'
)
...
def main():
    ...
    pmids = esearch_pmids(HELMINTHS_MA_TERM, args.retmax, args.email)
    print(f"helminths MA PMIDs: {len(pmids)}")
