"""
Validate the diabetes extractor against PUBLISHED type-2 diabetes meta-analyses
(silver standard).

Reuses the malaria MA-validation machinery: for each published T2DM RCT
meta-analysis, treat every reported `value (95% CI lo-hi)` as a reviewer datum and
measure how many our extractor recovers (point AND CI agree). Separates
comparative effect estimates (HR/OR/RR/MD) from prevalences/proportions.

Usage:
  python scripts/diabetes/validate_diabetes_ma.py --retmax 200 --email you@org
"""
import argparse
import json
import sys
...
OUT = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "diabetes" / "ma_validation.json"

DIABETES_MA_TERM = (
    '("type 2 diabetes" OR "type 2 diabetes mellitus" OR T2DM) AND '
    '(meta-analysis[Publication Type] OR systematic review[Publication Type]) '
    'AND (SGLT2 OR "GLP-1" OR metformin OR HbA1c OR cardiovascular OR renal '
    'OR randomized OR randomised OR efficacy OR trial)'
)
...
def main():
    ...
    pmids = esearch_pmids(DIABETES_MA_TERM, args.retmax, args.email)
    print(f"diabetes MA PMIDs: {len(pmids)}")
