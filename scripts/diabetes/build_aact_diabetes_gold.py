"""
Build an AACT gold file of type-2 diabetes trial effect estimates (reuses the
malaria AACT dialect + param_type normalization). Diabetes is the OPPOSITE of
typhoid for AACT: large pharma CVOTs (EMPA-REG, CANVAS, DECLARE, LEADER, etc.)
post rich numeric results on ClinicalTrials.gov, so this gold is dense.

Usage:
  python scripts/diabetes/build_aact_diabetes_gold.py --aact "F:/AACT-storage/AACT/2026-04-12"
"""
import argparse
import io
import json
...
OUT = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "diabetes" / "aact_diabetes_gold.json"
...
def main():
    ...
    ap.add_argument("--limit", type=int, default=600, help="cap NCTs (T2DM is huge)")
    args = ap.parse_args()
    ...
    SELECT DISTINCT nct_id FROM (
        SELECT nct_id FROM {rc(aact + '/conditions.txt')}
          WHERE downcase_name LIKE '%type 2 diabetes%' OR downcase_name LIKE '%type 2 diabetes mellitus%'
             OR downcase_name LIKE '%t2dm%' OR downcase_name = 'diabetes mellitus, type 2'
        UNION
        SELECT nct_id FROM {rc(aact + '/browse_conditions.txt')}
          WHERE downcase_mesh_term LIKE '%diabetes mellitus, type 2%'
    )
    """
    ncts = [r[0] for r in con.execute(nct_q).fetchall() if r[0]]
    print(f"type-2 diabetes NCTs in AACT: {len(ncts)}")
    if not ncts:
        OUT.write_text(json.dumps({"source": "AACT", "n_studies": 0, "n_effects": 0,
                                   "n_typed_effects": 0, "studies": []}, indent=2),
                       encoding="utf-8")
        sys.exit("no diabetes NCTs in AACT")
    if args.limit:
