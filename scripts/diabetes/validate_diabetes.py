"""
Diabetes extractor validation on the corpus: extraction yield, 2x2 / continuous
yield, and internal-consistency rate over abstracts (and PDFs with --pdfs).

Usage:
  python scripts/diabetes/validate_diabetes.py --limit 4000
  python scripts/diabetes/validate_diabetes.py --pdfs        # use downloaded full-text PDFs
"""
import argparse
import io
import json
...
from rct_extractor._engine.specialties.diabetes_arm_data import extract_arm_level
...
def main():
    ...
    eff_consistent = 0
    types = Counter(); diabetes_subspec = Counter()
    ...
    res = {
        ...
        "diabetes_subspecialty": dict(diabetes_subspec.most_common()),
        "arm_level": {
            "proportions": n_props,
