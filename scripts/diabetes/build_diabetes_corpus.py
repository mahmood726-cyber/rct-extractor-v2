"""
Build the type-2 diabetes RCT corpus index from PubMed (reuses the malaria corpus
helpers).

Output: data/field_portability/diabetes/diabetes_matched.jsonl
  {study_id, pmid, pmcid, doi, nct_id, other_registry_ids, title, journal, year,
   abstract, has_pdf}

T2DM is a very large field; --retmax bounds the pull (default 4000 most-recent).

Usage:
  python scripts/diabetes/build_diabetes_corpus.py --retmax 4000 --email you@org
"""
import argparse
import io
import json
...
OUT_DIR = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "diabetes"
OUT_FILE = OUT_DIR / "diabetes_matched.jsonl"

DIABETES_TERM = (
    '("type 2 diabetes"[Title/Abstract] OR "type 2 diabetes mellitus"[Title/Abstract] OR '
    'T2DM[Title/Abstract] OR "Diabetes Mellitus, Type 2"[MeSH Terms]) '
...
def main():
    ap = argparse.ArgumentParser(description="Build type-2 diabetes RCT corpus index")
    ap.add_argument("--term", default=DIABETES_TERM)
    ...
    print(f"esearch type-2 diabetes RCTs (retmax={args.retmax})...")
