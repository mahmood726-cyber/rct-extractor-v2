"""
Build the lymphoma RCT corpus index from PubMed (reuses malaria helpers).
Output: data/field_portability/lymphoma/lymphoma_matched.jsonl
Usage: python scripts/lymphoma/build_lymphoma_corpus.py --retmax 3000
"""
...
OUT_DIR = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "lymphoma"
OUT_FILE = OUT_DIR / "lymphoma_matched.jsonl"

LYMPHOMA_TERM = (
    '("lymphoma"[Title/Abstract] OR "Hodgkin lymphoma"[Title/Abstract] '
    'OR "non-Hodgkin lymphoma"[Title/Abstract] OR "diffuse large B-cell lymphoma"[Title/Abstract] '
...
def main():
    ap = argparse.ArgumentParser(description="Build lymphoma RCT corpus index")
    ap.add_argument("--term", default=LYMPHOMA_TERM)
    ...
    print(f"esearch lymphoma RCTs (retmax={args.retmax})...")
    ...
    for bi, batch in enumerate(chunks(pmids, args.batch)):
        ...
        for pmid in batch:
            ...
            ids = idmap.get(pmid, {})
            records[pmid] = {
                "study_id": f"lymphoma_{pmid}", "pmid": pmid,
