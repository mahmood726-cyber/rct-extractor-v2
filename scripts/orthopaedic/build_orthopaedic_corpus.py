"""
Build the orthopaedic / fracture-surgery RCT corpus index from PubMed
(reuses malaria helpers).

Output: data/field_portability/orthopaedic/orthopaedic_matched.jsonl

Usage:
  python scripts/orthopaedic/build_orthopaedic_corpus.py --retmax 4000 --email you@org
"""
import argparse
import io
import json
...
OUT_DIR = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "orthopaedic"
OUT_FILE = OUT_DIR / "orthopaedic_matched.jsonl"

ORTHOPAEDIC_TERM = (
    '("fracture fixation"[Title/Abstract] OR "internal fixation"[Title/Abstract] '
    'OR "intramedullary nail"[Title/Abstract] OR "open reduction"[Title/Abstract] '
    'OR arthroplasty[Title/Abstract] OR "total hip replacement"[Title/Abstract] '
    'OR "total knee replacement"[Title/Abstract] OR hemiarthroplasty[Title/Abstract] '
    'OR nonunion[Title/Abstract] OR "ACL reconstruction"[Title/Abstract] '
    'OR "rotator cuff repair"[Title/Abstract]) '
...
def main():
    ap = argparse.ArgumentParser(description="Build orthopaedic RCT corpus index")
    ap.add_argument("--term", default=ORTHOPAEDIC_TERM)
    ...
    print(f"esearch orthopaedic RCTs (retmax={args.retmax})...")
