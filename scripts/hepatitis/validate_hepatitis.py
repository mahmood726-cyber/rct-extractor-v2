"""
Hepatitis extractor validation on the corpus: extraction yield, 2x2 / continuous
yield, and internal-consistency rate over abstracts (and PDFs with --pdfs).

Usage:
  python scripts/hepatitis/validate_hepatitis.py --limit 3000
  python scripts/hepatitis/validate_hepatitis.py --pdfs        # use downloaded full-text PDFs
"""
import argparse
import io
import json
...
from rct_extractor._engine.specialties.hepatitis_arm_data import extract_arm_level
from rct_extractor._engine.specialties.registry import detect_specialty

HEP = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "hepatitis"
MATCHED = HEP / "hepatitis_matched.jsonl"
PDF_DIR = HEP / "rct_trial_pdfs"
OUT = HEP / "validation.json"
...
def main():
    ...
    eff_consistent = 0
    types = Counter(); hep_subspec = Counter()
    ...
    res = {
        ...
        "hepatitis_subspecialty": dict(hep_subspec.most_common()),
        "arm_level": {
            "proportions": n_props,
