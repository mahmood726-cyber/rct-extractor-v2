"""
Build an AACT gold file of hepatitis trial effect estimates (reuses the malaria
AACT dialect + param_type normalization). Hepatitis RCTs (especially HCV DAA
registration trials) are heavily registered with posted results.

Usage:
  python scripts/hepatitis/build_aact_hepatitis_gold.py --aact "F:/AACT-storage/AACT/2026-04-12"
"""
import argparse
import io
import json
...
OUT = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "hepatitis" / "aact_hepatitis_gold.json"
...
def main():
    ...
    SELECT DISTINCT nct_id FROM (
        SELECT nct_id FROM {rc(aact + '/conditions.txt')}
          WHERE downcase_name LIKE '%hepatitis b%' OR downcase_name LIKE '%hepatitis c%'
             OR downcase_name LIKE '%chronic hepatitis%' OR downcase_name LIKE '%hbv%'
             OR downcase_name LIKE '%hcv%'
        UNION
        SELECT nct_id FROM {rc(aact + '/browse_conditions.txt')}
          WHERE downcase_mesh_term LIKE '%hepatitis b%' OR downcase_mesh_term LIKE '%hepatitis c%'
             OR downcase_mesh_term LIKE '%hepatitis, chronic%'
    )
    """
    ncts = [r[0] for r in con.execute(nct_q).fetchall() if r[0]]
    print(f"Hepatitis NCTs in AACT: {len(ncts)}")
    con.execute("CREATE TEMP TABLE hn AS SELECT * FROM (VALUES "
    ...
    JOIN hn m ON oa.nct_id = m.nct_id
    ...
    titles = {r[0]: r[1] for r in con.execute(
        f"SELECT t.nct_id, t.brief_title FROM {rc(aact + '/studies.txt')} t JOIN hn m ON t.nct_id=m.nct_id"
    ).fetchall()}
