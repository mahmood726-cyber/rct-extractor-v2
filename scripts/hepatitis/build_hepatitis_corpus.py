"""
Build the hepatitis (HBV/HCV) RCT corpus index from PubMed (reuses the malaria
corpus helpers).

Output: data/field_portability/hepatitis/hepatitis_matched.jsonl
  {study_id, pmid, pmcid, doi, nct_id, other_registry_ids, title, journal, year,
   abstract, has_pdf}

Usage:
  python scripts/hepatitis/build_hepatitis_corpus.py --retmax 3000 --email you@org
"""
import argparse
import io
import json
...
OUT_DIR = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "hepatitis"
OUT_FILE = OUT_DIR / "hepatitis_matched.jsonl"

HEPATITIS_TERM = (
    '("hepatitis B"[Title/Abstract] OR "hepatitis C"[Title/Abstract] OR '
    'HBV[Title/Abstract] OR HCV[Title/Abstract]) '
...
def main():
    ap = argparse.ArgumentParser(description="Build hepatitis RCT corpus index")
    ap.add_argument("--term", default=HEPATITIS_TERM)
    ...
    print(f"esearch hepatitis RCTs (retmax={args.retmax})...")
