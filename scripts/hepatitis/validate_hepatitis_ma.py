"""
Validate the hepatitis extractor against PUBLISHED hepatitis meta-analyses
(silver standard).

Reuses the malaria MA-validation machinery: for each published hepatitis RCT
...
Usage:
  python scripts/hepatitis/validate_hepatitis_ma.py --retmax 150 --email you@org
"""
import argparse
import json
import sys
...
OUT = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "hepatitis" / "ma_validation.json"

HEPATITIS_MA_TERM = (
    '(hepatitis B OR hepatitis C OR HBV OR HCV) '
    'AND (meta-analysis[Publication Type] OR systematic review[Publication Type]) '
    'AND (antiviral OR randomized OR randomised OR efficacy OR trial)'
)
...
def main():
    ...
    pmids = esearch_pmids(HEPATITIS_MA_TERM, args.retmax, args.email)
    print(f"Hepatitis MA PMIDs: {len(pmids)}")
