"""
Build the influenza RCT corpus index from PubMed (reuses malaria helpers).
Output: data/field_portability/influenza/influenza_matched.jsonl
Usage: python scripts/influenza/build_influenza_corpus.py --retmax 3000
"""
...
OUT_DIR = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "influenza"
OUT_FILE = OUT_DIR / "influenza_matched.jsonl"

INFLUENZA_TERM = (
    '("influenza"[Title/Abstract] OR "oseltamivir"[Title/Abstract] '
    'OR "baloxavir"[Title/Abstract] OR "influenza vaccine"[Title/Abstract] '
    'OR "neuraminidase inhibitor"[Title/Abstract]) '
...
def main():
    ap = argparse.ArgumentParser(description="Build influenza RCT corpus index")
    ap.add_argument("--term", default=INFLUENZA_TERM)
    ...
    print(f"esearch influenza RCTs (retmax={args.retmax})...")
    ...
    for bi, batch in enumerate(chunks(pmids, args.batch)):
        ...
        for pmid in batch:
            ...
            ids = idmap.get(pmid, {})
            records[pmid] = {
                "study_id": f"influenza_{pmid}", "pmid": pmid,
