"""
Build the ARDS / acute-respiratory-failure RCT corpus index from PubMed
(reuses the malaria corpus helpers).

Output: data/field_portability/ards/ards_matched.jsonl
  {study_id, pmid, pmcid, doi, nct_id, other_registry_ids, title, journal, year,
   abstract, has_pdf}

Usage:
  python scripts/ards/build_ards_corpus.py --retmax 4000 --email you@org
"""
import argparse
import io
import json
...
OUT_DIR = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "ards"
OUT_FILE = OUT_DIR / "ards_matched.jsonl"

ARDS_TERM = (
    '("acute respiratory distress syndrome"[Title/Abstract] OR \"ARDS\"[Title/Abstract] '
    'OR "acute lung injury"[Title/Abstract] OR "acute respiratory failure"[Title/Abstract] '
    'OR "acute hypoxemic respiratory failure"[Title/Abstract] OR '
    '"mechanical ventilation"[Title/Abstract] OR "prone positioning"[Title/Abstract] '
    'OR "lung-protective ventilation"[Title/Abstract] OR "ventilator-free days"[Title/Abstract]) '
...
def main():
    ap = argparse.ArgumentParser(description="Build ARDS RCT corpus index")
    ap.add_argument("--term", default=ARDS_TERM)
    ...
    print(f"esearch ARDS RCTs (retmax={args.retmax})...")
