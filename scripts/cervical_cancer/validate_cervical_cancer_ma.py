"""
Validate the cervical-cancer / HPV extractor against PUBLISHED cervical-cancer /
HPV meta-analyses (silver standard).

Reuses the malaria MA-validation machinery: for each published cervical-cancer /
HPV RCT meta-analysis, treat every reported `value (95% CI lo-hi)` as a reviewer
datum and measure how many our extractor recovers (point AND CI agree). Separates
comparative effect estimates (HR/OR/RR/IRR/MD) from prevalences/proportions.

Usage:
  python scripts/cervical_cancer/validate_cervical_cancer_ma.py --retmax 150 --email you@org
"""
import argparse
import json
import sys
...
OUT = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "cervical_cancer" / "ma_validation.json"

CERVICAL_CANCER_MA_TERM = (
    '("cervical cancer" OR "cervical intraepithelial neoplasia" OR "HPV vaccine" '
    'OR "human papillomavirus" OR "cervical screening") AND '
    '(meta-analysis[Publication Type] OR systematic review[Publication Type]) '
    'AND (vaccine OR screening OR cryotherapy OR ablation OR efficacy OR randomized '
...
def main():
    ...
    pmids = esearch_pmids(CERVICAL_CANCER_MA_TERM, args.retmax, args.email)
    print(f"cervical-cancer / HPV MA PMIDs: {len(pmids)}")
