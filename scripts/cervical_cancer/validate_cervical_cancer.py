"""
Cervical-cancer / HPV extractor validation on the corpus: extraction yield, 2x2 /
continuous yield, and internal-consistency rate over abstracts (and PDFs with
--pdfs).

Usage:
  python scripts/cervical_cancer/validate_cervical_cancer.py --limit 3000
  python scripts/cervical_cancer/validate_cervical_cancer.py --pdfs   # use downloaded full-text PDFs
"""
import argparse
import io
import json
...
from rct_extractor._engine.specialties.cervical_cancer_arm_data import extract_arm_level
from rct_extractor._engine.specialties.registry import detect_specialty

CC = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "cervical_cancer"
MATCHED = CC / "cervical_cancer_matched.jsonl"
PDF_DIR = CC / "rct_trial_pdfs"
OUT = CC / "validation.json"
...
def main():
    ...
    eff_consistent = 0
    types = Counter(); cc_subspec = Counter()
    ...
    res = {
        ...
        "cervical_cancer_subspecialty": dict(cc_subspec.most_common()),
        "arm_level": {
            "proportions": n_props,
