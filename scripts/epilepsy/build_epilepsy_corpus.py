"""
Build the epilepsy / AED RCT corpus index from PubMed (reuses the malaria corpus
helpers).

Output: data/field_portability/epilepsy/epilepsy_matched.jsonl
  {study_id, pmid, pmcid, doi, nct_id, other_registry_ids, title, journal, year,
   abstract, has_pdf}

Usage:
  python scripts/epilepsy/build_epilepsy_corpus.py --retmax 3000 --email you@org
"""
import argparse
import io
import json
...
OUT_DIR = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "epilepsy"
OUT_FILE = OUT_DIR / "epilepsy_matched.jsonl"

EPILEPSY_TERM = (
    '(epilepsy[Title/Abstract] OR seizure[Title/Abstract] OR '
    '"status epilepticus"[Title/Abstract] OR antiepileptic[Title/Abstract] OR '
    'anticonvulsant[Title/Abstract] OR antiseizure[Title/Abstract]) '
...
def main():
    ap = argparse.ArgumentParser(description="Build epilepsy RCT corpus index")
    ap.add_argument("--term", default=EPILEPSY_TERM)
    ...
    print(f"esearch epilepsy RCTs (retmax={args.retmax})...")
