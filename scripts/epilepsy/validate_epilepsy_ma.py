"""
Validate the epilepsy extractor against PUBLISHED epilepsy / AED meta-analyses
(silver standard).

Reuses the malaria MA-validation machinery: for each published epilepsy RCT
...
Usage:
  python scripts/epilepsy/validate_epilepsy_ma.py --retmax 150 --email you@org
"""
import argparse
import json
import sys
...
OUT = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "epilepsy" / "ma_validation.json"

EPILEPSY_MA_TERM = (
    '(epilepsy OR seizure OR "status epilepticus" OR antiepileptic OR anticonvulsant) AND '
    '(meta-analysis[Publication Type] OR systematic review[Publication Type]) '
    'AND (efficacy OR responder OR "seizure freedom" OR withdrawal OR tolerability '
    'OR randomized OR randomised OR trial)'
)
...
def main():
    ...
    pmids = esearch_pmids(EPILEPSY_MA_TERM, args.retmax, args.email)
    print(f"epilepsy MA PMIDs: {len(pmids)}")
