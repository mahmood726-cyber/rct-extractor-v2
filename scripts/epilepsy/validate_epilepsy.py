"""
Epilepsy extractor validation on the corpus: extraction yield, 2x2 / continuous
yield, and internal-consistency rate over abstracts (and PDFs with --pdfs).

Usage:
  python scripts/epilepsy/validate_epilepsy.py --limit 3000
  python scripts/epilepsy/validate_epilepsy.py --pdfs        # use downloaded full-text PDFs
"""
import argparse
import io
import json
...
from src.specialties.epilepsy_arm_data import extract_arm_level
from src.specialties.registry import detect_specialty

EPILEPSY = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "epilepsy"
MATCHED = EPILEPSY / "epilepsy_matched.jsonl"
PDF_DIR = EPILEPSY / "rct_trial_pdfs"
OUT = EPILEPSY / "validation.json"
...
def main():
    ...
    eff_consistent = 0
    types = Counter(); epilepsy_subspec = Counter()
    ...
    res = {
        ...
        "epilepsy_subspecialty": dict(epilepsy_subspec.most_common()),
        "arm_level": {
            "proportions": n_props,
