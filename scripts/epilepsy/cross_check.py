"""
Epilepsy unified cross-check: per trial reconcile PDF extractions vs PubMed
abstract vs AACT (when NCT). Abstract->PDF recall is the at-scale signal;
AACT->PDF is the independent numeric gold (richer for epilepsy than typhoid/cholera).

Outputs: data/field_portability/epilepsy/cross_check.json
Usage: python scripts/epilepsy/cross_check.py --limit 0
"""
import argparse
import io
import json
...
AACT_GOLD = EPILEPSY / "aact_epilepsy_gold.json"
PDF_DIR = EPILEPSY / "rct_trial_pdfs"
OUT = EPILEPSY / "cross_check.json"
