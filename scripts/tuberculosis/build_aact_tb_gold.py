"""
Build an AACT gold file of tuberculosis trial effect estimates (reuses the
malaria AACT dialect + param_type normalization). Many TB RCTs (especially MDR-TB
and TB-vaccine trials) register with posted results.

Usage:
  python scripts/tuberculosis/build_aact_tb_gold.py --aact "F:/AACT-storage/AACT/2026-04-12"
"""
import argparse
import io
import json
...
OUT = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "tuberculosis" / "aact_tb_gold.json"
...
def main():
    ...
    SELECT DISTINCT nct_id FROM (
        SELECT nct_id FROM {rc(aact + '/conditions.txt')}
          WHERE downcase_name LIKE '%tuberculosis%' OR downcase_name LIKE '%mycobacterium tuberculosis%'
             OR downcase_name LIKE '%mdr-tb%' OR downcase_name LIKE '%latent tuberculosis%'
        UNION
        SELECT nct_id FROM {rc(aact + '/browse_conditions.txt')}
          WHERE downcase_mesh_term LIKE '%tuberculosis%'
    )
    """
    ncts = [r[0] for r in con.execute(nct_q).fetchall() if r[0]]
    print(f"TB NCTs in AACT: {len(ncts)}")
