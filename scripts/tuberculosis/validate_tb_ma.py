"""
Validate the TB extractor against PUBLISHED tuberculosis meta-analyses (silver
standard).

Reuses the malaria MA-validation machinery: for each published TB RCT
...
Usage:
  python scripts/tuberculosis/validate_tb_ma.py --retmax 150 --email you@org
"""
import argparse
import json
import sys
...
OUT = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "tuberculosis" / "ma_validation.json"

TB_MA_TERM = (
    'tuberculosis AND (meta-analysis[Publication Type] OR systematic review[Publication Type]) '
    'AND (treatment OR randomized OR randomised OR efficacy OR trial OR regimen)'
)
...
def main():
    ...
    pmids = esearch_pmids(TB_MA_TERM, args.retmax, args.email)
    print(f"TB MA PMIDs: {len(pmids)}")
