"""
Build the tuberculosis RCT corpus index from PubMed (reuses the malaria corpus
helpers).

Output: data/field_portability/tuberculosis/tuberculosis_matched.jsonl
...
OUT_DIR = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "tuberculosis"
OUT_FILE = OUT_DIR / "tuberculosis_matched.jsonl"

TB_TERM = (
    '(tuberculosis[Title/Abstract] OR "M. tuberculosis"[Title/Abstract] OR '
    '"TB preventive"[Title/Abstract]) '
...
def main():
    ap = argparse.ArgumentParser(description="Build tuberculosis RCT corpus index")
    ap.add_argument("--term", default=TB_TERM)
    ...
    print(f"esearch TB RCTs (retmax={args.retmax})...")
