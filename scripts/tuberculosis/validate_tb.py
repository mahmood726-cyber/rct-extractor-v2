"""
TB extractor validation on the corpus: extraction yield, 2x2 / continuous yield,
and internal-consistency rate over abstracts (and PDFs with --pdfs).

Usage:
  python scripts/tuberculosis/validate_tb.py --limit 3000
  python scripts/tuberculosis/validate_tb.py --pdfs        # use downloaded full-text PDFs
"""
import argparse
import io
import json
...
from rct_extractor._engine.specialties.tuberculosis_arm_data import extract_arm_level
...
PDF_DIR = TB / "rct_trial_pdfs"
OUT = TB / "validation.json"
...
def main():
    ...
    eff_consistent = 0
    types = Counter(); tb_subspec = Counter()
    ...
    res = {
        ...
        "tb_subspecialty": dict(tb_subspec.most_common()),
        "arm_level": {
            "proportions": n_props,
