"""
Build the sepsis RCT corpus index from PubMed (reuses malaria corpus helpers).
Canonical NCBI query is SEPSIS_TERM. NCBI eutils was DNS-blocked on the build
host; real-PDF gold acquired via the EuropePMC harness
(scripts/pdf_eval/acquire_epmc_gold.py). Builder kept for parity.
Output: data/field_portability/sepsis/sepsis_matched.jsonl
"""
...
OUT_DIR = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "sepsis"
OUT_FILE = OUT_DIR / "sepsis_matched.jsonl"
SEPSIS_TERM = (
    '(sepsis[Title/Abstract] OR "septic shock"[Title/Abstract] '
    'OR septicaemia[Title/Abstract] OR septicemia[Title/Abstract]) '
...
def main():
    ap = argparse.ArgumentParser(description="Build sepsis RCT corpus index")
    ap.add_argument("--term", default=SEPSIS_TERM)
