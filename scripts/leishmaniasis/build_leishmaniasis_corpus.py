"""
Build the leishmaniasis RCT corpus index from PubMed (reuses the malaria corpus helpers).

Output: data/field_portability/leishmaniasis/leishmaniasis_matched.jsonl
  {study_id, pmid, pmcid, doi, nct_id, other_registry_ids, title, journal, year,
   abstract, has_pdf}

Usage:
  python scripts/leishmaniasis/build_leishmaniasis_corpus.py --retmax 4000 --email you@org
"""
import argparse
import io
import json
...
OUT_DIR = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "leishmaniasis"
OUT_FILE = OUT_DIR / "leishmaniasis_matched.jsonl"

LEISH_TERM = (
    '(leishmaniasis[Title/Abstract] OR leishmania[Title/Abstract] OR '
    'kala-azar[Title/Abstract] OR "visceral leishmaniasis"[Title/Abstract] OR '
    '"cutaneous leishmaniasis"[Title/Abstract] OR miltefosine[Title/Abstract] OR '
    'paromomycin[Title/Abstract] OR "sodium stibogluconate"[Title/Abstract]) '
...
def main():
    ap = argparse.ArgumentParser(description="Build leishmaniasis RCT corpus index")
    ap.add_argument("--term", default=LEISH_TERM)
    ...
    print(f"esearch leishmaniasis RCTs (retmax={args.retmax})...")
