"""
Leishmaniasis extractor validation on the corpus: extraction yield, 2x2 /
continuous yield, and internal-consistency rate over abstracts (and PDFs with
--pdfs).

Usage:
  python scripts/leishmaniasis/validate_leishmaniasis.py --limit 4000
  python scripts/leishmaniasis/validate_leishmaniasis.py --pdfs   # use downloaded full-text PDFs
"""
import argparse
import io
import json
...
from src.specialties.leishmaniasis_arm_data import extract_arm_level
from src.specialties.registry import detect_specialty

LEISH = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "leishmaniasis"
MATCHED = LEISH / "leishmaniasis_matched.jsonl"
PDF_DIR = LEISH / "rct_trial_pdfs"
OUT = LEISH / "validation.json"
...
def main():
    ...
    eff_consistent = 0
    types = Counter(); leish_subspec = Counter()
    ...
    res = {
        ...
        "leishmaniasis_subspecialty": dict(leish_subspec.most_common()),
        "arm_level": {
            "proportions": n_props,
