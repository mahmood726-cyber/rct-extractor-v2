"""
Validate the leishmaniasis extractor against PUBLISHED leishmaniasis
meta-analyses (silver standard).

Reuses the malaria MA-validation machinery: for each published leishmaniasis RCT
...
Usage:
  python scripts/leishmaniasis/validate_leishmaniasis_ma.py --retmax 200 --email you@org
"""
import argparse
import json
import sys
...
OUT = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "leishmaniasis" / "ma_validation.json"

LEISH_MA_TERM = (
    '(leishmaniasis OR leishmania OR kala-azar) AND '
    '(meta-analysis[Publication Type] OR systematic review[Publication Type]) '
    'AND (miltefosine OR amphotericin OR paromomycin OR stibogluconate OR '
    'antimonial OR randomized OR randomised OR efficacy OR trial OR cure)'
)
...
def main():
    ...
    pmids = esearch_pmids(LEISH_MA_TERM, args.retmax, args.email)
    print(f"leishmaniasis MA PMIDs: {len(pmids)}")
