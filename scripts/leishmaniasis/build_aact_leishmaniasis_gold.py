"""
Build an AACT gold file of leishmaniasis trial effect estimates (reuses the
malaria AACT dialect + param_type normalization). Many leishmaniasis trials
register on ISRCTN / PACTR / CTRI rather than ClinicalTrials.gov, so this gold may
be sparser than HIV's.

Usage:
  python scripts/leishmaniasis/build_aact_leishmaniasis_gold.py --aact "F:/AACT-storage/AACT/2026-04-12"
"""
import argparse
import io
import json
...
OUT = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "leishmaniasis" / "aact_leishmaniasis_gold.json"
