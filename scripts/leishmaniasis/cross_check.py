"""
Leishmaniasis unified cross-check: per trial reconcile PDF extractions vs PubMed
abstract vs AACT (when NCT). Abstract->PDF recall is the at-scale signal;
AACT->PDF is the independent numeric gold (sparse for leishmaniasis).

Outputs: data/field_portability/leishmaniasis/cross_check.json
Usage: python scripts/leishmaniasis/cross_check.py --limit 0
"""
import argparse
import io
import json
...
AACT_GOLD = LEISH / "aact_leishmaniasis_gold.json"
PDF_DIR = LEISH / "rct_trial_pdfs"
OUT = LEISH / "cross_check.json"
