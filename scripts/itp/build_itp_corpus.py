"""
Build the ITP (immune thrombocytopenia) RCT corpus index from PubMed
(reuses malaria helpers).

Output: data/field_portability/itp/itp_matched.jsonl

Usage:
  python scripts/itp/build_itp_corpus.py --retmax 4000 --email you@org
"""
import argparse
import io
import json
...
OUT_DIR = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "itp"
OUT_FILE = OUT_DIR / "itp_matched.jsonl"

ITP_TERM = (
    '("immune thrombocytopenia"[Title/Abstract] OR "immune thrombocytopenic purpura"[Title/Abstract] '
    'OR "idiopathic thrombocytopenic purpura"[Title/Abstract] OR ITP[Title/Abstract] '
    'OR eltrombopag[Title/Abstract] OR romiplostim[Title/Abstract] OR avatrombopag[Title/Abstract] '
    'OR fostamatinib[Title/Abstract] OR "thrombopoietin receptor agonist"[Title/Abstract] '
    'OR "platelet response"[Title/Abstract]) '
...
def main():
    ap = argparse.ArgumentParser(description="Build ITP RCT corpus index")
    ap.add_argument("--term", default=ITP_TERM)
    ...
    print(f"esearch ITP RCTs (retmax={args.retmax})...")
