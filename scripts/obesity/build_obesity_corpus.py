"""
Build the obesity / weight-management RCT corpus index from PubMed
(reuses the malaria corpus helpers).

Output: data/field_portability/obesity/obesity_matched.jsonl
...
retained for the canonical eutils pipeline and exposes OBESITY_TERM.

Usage:
  python scripts/obesity/build_obesity_corpus.py --retmax 4000
"""
import argparse
import io
import json
...
OUT_DIR = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "obesity"
OUT_FILE = OUT_DIR / "obesity_matched.jsonl"

OBESITY_TERM = (
    '(obesity[Title/Abstract] OR overweight[Title/Abstract] OR "weight loss"[Title/Abstract] OR '
    '"weight management"[Title/Abstract] OR "body weight"[Title/Abstract] OR '
    'semaglutide[Title/Abstract] OR tirzepatide[Title/Abstract] OR liraglutide[Title/Abstract] OR '
    'orlistat[Title/Abstract] OR "anti-obesity"[Title/Abstract]) '
...
def main():
    ap = argparse.ArgumentParser(description="Build obesity RCT corpus index")
    ap.add_argument("--term", default=OBESITY_TERM)
    ...
    print(f"esearch obesity RCTs (retmax={args.retmax})...")
