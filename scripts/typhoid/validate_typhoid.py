"""
Typhoid extractor validation on the corpus: extraction yield, 2x2 / continuous
yield, and internal-consistency rate over abstracts (and PDFs with --pdfs).

Usage:
  python scripts/typhoid/validate_typhoid.py --limit 3000
  python scripts/typhoid/validate_typhoid.py --pdfs        # use downloaded full-text PDFs
"""
import argparse
import io
import json
...
from rct_extractor._engine.specialties.typhoid_arm_data import extract_arm_level
from rct_extractor._engine.specialties.registry import detect_specialty

TYPHOID = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "typhoid"
MATCHED = TYPHOID / "typhoid_matched.jsonl"
PDF_DIR = TYPHOID / "rct_trial_pdfs"
OUT = TYPHOID / "validation.json"
...
def main():
    ...
    eff_consistent = 0
    types = Counter(); typhoid_subspec = Counter()
    ...
    res = {
        ...
        "typhoid_subspecialty": dict(typhoid_subspec.most_common()),
        "arm_level": {
            "proportions": n_props,
