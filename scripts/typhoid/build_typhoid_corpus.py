"""
Build the typhoid RCT corpus index from PubMed (reuses the malaria corpus helpers).

Output: data/field_portability/typhoid/typhoid_matched.jsonl
  {study_id, pmid, pmcid, doi, nct_id, other_registry_ids, title, journal, year,
   abstract, has_pdf}

Usage:
  python scripts/typhoid/build_typhoid_corpus.py --retmax 3000 --email you@org
"""
import argparse
import io
import json
...
OUT_DIR = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "typhoid"
OUT_FILE = OUT_DIR / "typhoid_matched.jsonl"

TYPHOID_TERM = (
    '(typhoid[Title/Abstract] OR "enteric fever"[Title/Abstract] OR '
    'paratyphoid[Title/Abstract] OR "Salmonella Typhi"[Title/Abstract] OR '
    '"typhoid conjugate vaccine"[Title/Abstract]) '
...
def main():
    ap = argparse.ArgumentParser(description="Build typhoid RCT corpus index")
    ap.add_argument("--term", default=TYPHOID_TERM)
    ...
    print(f"esearch typhoid RCTs (retmax={args.retmax})...")
