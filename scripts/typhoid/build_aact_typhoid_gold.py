"""
Build an AACT gold file of typhoid trial effect estimates (reuses the malaria
AACT dialect + param_type normalization). Many typhoid trials register on ISRCTN
/ CTRI rather than ClinicalTrials.gov, so this gold may be sparser than HIV's.

Usage:
  python scripts/typhoid/build_aact_typhoid_gold.py --aact "F:/AACT-storage/AACT/2026-04-12"
"""
import argparse
import io
import json
...
OUT = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "typhoid" / "aact_typhoid_gold.json"
...
def main():
    ...
    SELECT DISTINCT nct_id FROM (
        SELECT nct_id FROM {rc(aact + '/conditions.txt')}
          WHERE downcase_name LIKE '%typhoid%' OR downcase_name LIKE '%enteric fever%'
             OR downcase_name LIKE '%paratyphoid%' OR downcase_name LIKE '%salmonella typhi%'
        UNION
        SELECT nct_id FROM {rc(aact + '/browse_conditions.txt')}
          WHERE downcase_mesh_term LIKE '%typhoid%' OR downcase_mesh_term LIKE '%paratyphoid%'
    )
    """
    ncts = [r[0] for r in con.execute(nct_q).fetchall() if r[0]]
    print(f"typhoid NCTs in AACT: {len(ncts)}")
    if not ncts:
        OUT.write_text(json.dumps({"source": "AACT", "n_studies": 0, "n_effects": 0,
                                   "n_typed_effects": 0, "studies": []}, indent=2),
                       encoding="utf-8")
        sys.exit("no typhoid NCTs in AACT")
