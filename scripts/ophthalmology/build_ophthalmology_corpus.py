"""
Build the ophthalmology (AMD / DME / glaucoma / dry eye) RCT corpus index from
PubMed (reuses the malaria corpus helpers).

Output: data/field_portability/ophthalmology/ophthalmology_matched.jsonl
  {study_id, pmid, pmcid, doi, title, journal, year, abstract, has_pdf}

Usage:
  python scripts/ophthalmology/build_ophthalmology_corpus.py --retmax 3000 --email you@org
"""
import argparse
import io
import json
...
OUT_DIR = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "ophthalmology"
OUT_FILE = OUT_DIR / "ophthalmology_matched.jsonl"

OPHTHALMOLOGY_TERM = (
    '("age-related macular degeneration"[Title/Abstract] OR '
    '"macular degeneration"[Title/Abstract] OR '
    '"diabetic macular edema"[Title/Abstract] OR '
    '"diabetic macular oedema"[Title/Abstract] OR '
    '"diabetic retinopathy"[Title/Abstract] OR '
    'glaucoma[Title/Abstract] OR '
    '"ocular hypertension"[Title/Abstract] OR '
    '"dry eye disease"[Title/Abstract] OR '
    '"dry eye"[Title/Abstract]) '
...
def main():
    ap = argparse.ArgumentParser(description="Build ophthalmology RCT corpus index")
    ap.add_argument("--term", default=OPHTHALMOLOGY_TERM)
