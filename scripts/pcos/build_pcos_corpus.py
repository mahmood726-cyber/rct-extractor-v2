"""
Build the PCOS RCT corpus index from PubMed (reuses malaria helpers).

Output: data/field_portability/pcos/pcos_matched.jsonl
...
retained for the canonical eutils pipeline and exposes PCOS_TERM.

Usage:
  python scripts/pcos/build_pcos_corpus.py --retmax 4000
"""
import argparse
import io
import json
...
OUT_DIR = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "pcos"
OUT_FILE = OUT_DIR / "pcos_matched.jsonl"

PCOS_TERM = (
    '("polycystic ovary syndrome"[Title/Abstract] OR PCOS[Title/Abstract] OR '
    '"polycystic ovarian syndrome"[Title/Abstract] OR "ovulation induction"[Title/Abstract] OR '
    'letrozole[Title/Abstract] OR clomiphene[Title/Abstract] OR clomifene[Title/Abstract] OR '
    'hirsutism[Title/Abstract]) '
...
def main():
    ap = argparse.ArgumentParser(description="Build PCOS RCT corpus index")
    ap.add_argument("--term", default=PCOS_TERM)
    ...
    print(f"esearch PCOS RCTs (retmax={args.retmax})...")
