"""
Build the thyroid cancer RCT corpus index from PubMed (reuses malaria helpers).
Output: data/field_portability/thyroid_cancer/thyroid_cancer_matched.jsonl
Usage: python scripts/thyroid_cancer/build_thyroid_cancer_corpus.py --retmax 3000
"""
...
OUT_DIR = Path(__file__).resolve().parents[2] / "data" / "field_portability" / "thyroid_cancer"
OUT_FILE = OUT_DIR / "thyroid_cancer_matched.jsonl"

THYROID_CANCER_TERM = (
    '("thyroid cancer"[Title/Abstract] OR "thyroid carcinoma"[Title/Abstract] '
    'OR "differentiated thyroid"[Title/Abstract] OR "medullary thyroid"[Title/Abstract] '
    'OR "anaplastic thyroid"[Title/Abstract]) '
...
def main():
    ap = argparse.ArgumentParser(description="Build thyroid cancer RCT corpus index")
    ap.add_argument("--term", default=THYROID_CANCER_TERM)
    ...
    print(f"esearch thyroid cancer RCTs (retmax={args.retmax})...")
    ...
    for bi, batch in enumerate(chunks(pmids, args.batch)):
        ...
        for pmid in batch:
            ...
            ids = idmap.get(pmid, {})
            records[pmid] = {
                "study_id": f"thyroid_cancer_{pmid}", "pmid": pmid,
