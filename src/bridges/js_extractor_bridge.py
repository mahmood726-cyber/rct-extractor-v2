...
# Path to the JS extractor
DEFAULT_JS_EXTRACTOR = r"C:\Users\user\Downloads\Dataextractor\RCTExtractor_v4_8_AI.js"
...
// Load the extractor (handle module format)
let AIStrategies;
try {{
    const extractor = require('{self.js_extractor_path.replace(chr(92), "/")}');
    AIStrategies = extractor.AIStrategies || extractor;
}} catch (e) {{
    // Try loading as script and extracting AIStrategies
    const code = fs.readFileSync('{self.js_extractor_path.replace(chr(92), "/")}', 'utf8');
    eval(code);
}}
