...
class ConfidenceCalibrator:
    ...
    def _determine_thresholds(self, bins: List[CalibrationBin]) -> Dict[str, float]:
        ...
        for target in target_accuracies:
            # Find lowest confidence where observed accuracy >= target
            threshold = 1.0  # Default to maximum

            # Sort bins by confidence descending
            for b in sorted(bins, key=lambda x: x.bin_start, reverse=True):
                if b.observed_accuracy >= target:
    ...
    def calibrate(self, raw_confidence: float) -> float:
        ...
        for i, b in enumerate(self.model.bins):
            if b.bin_start <= raw_confidence <= b.bin_end:
                ...
                if b.bin_end != b.bin_start:
                    t = (raw_confidence - b.bin_start) / (b.bin_end - b.bin_start)
                else:
                    t = 0.5

                # Use observed accuracy as the calibrated value
