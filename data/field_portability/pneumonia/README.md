...
| **Effect internal-consistency** | **87.1%** | of 2,560 abstract effects across all 2,424 abstracts (Altman-Bland / midpoint checks) |
| **Arm-level proportion consistency** | **98.9%** | reported % == 100·events/total (185/187 proportions) |
| **Subspecialty routing** | vaccine 649 / treatment 450 / general 286 / mortality 284 / severe 164 | of the 2,424 corpus abstracts detected as pneumonia |
...
- Pneumonia RCT abstracts are **effect-estimate-heavy** (incidence rate ratios,
  ...
  *expected* to be — 49 tables across 2,424 abstracts, only 2 poolable because
  abstracts rarely restate both arms' n/N adjacently).
- Effect coverage on abstracts is **27.5%** (666/2,424) — most abstracts that
  report no parenthetical effect estimate are conference-style or descriptive;
  this is the same coverage regime as the sibling profiles.
...
- Internal-consistency (87.1% across the full corpus) is a touch below HIV's
  (~93%); failures are dominated by abstract-only effects whose CI the abstract
  never restates, not by mis-reads.
